...
Иногда это вызывало проблемы и поэтому больше не поддерживается. Реализуйте методы делегата вручную — [Метододы делегата приложения](#implement-application-delegate-methods).
...
```
[Тихие пуши]((#silent-push-notifications)) не требуют разрешения.
...
По умолчанию SDK отслеживает токен push-уведомления только в том случае, если приложение авторизовано (если не включена [проверка настройки push](#configure-the-sdk)). Как отслеживать токен даже когда приложение не авторизовано читайте в разделе [Тихие Push-уведомления](#silent-push-notifications).
...
Чтобы приложение корректно реагировало на входящие push-уведомления, укажите делегата, который будет вызываться при открытии уведомления пользователем или при получении [тихое push-уведомление](#silent-push-notifications).

Делегат должен реализовать протокол `PushNotificationManagerDelegate` и метод `pushNotificationOpened`, которому SDK передаст выбранное пользователем действие и [полезную нагрузку уведомления](#payload-example). При необходимости вы также можете реализовать метод `silentPushNotificationReceived` для обработки [тихих push-уведомлений](#silent-push-notifications).
...
Для ответа на тихие уведомления установите `Sendsay.shared.pushNotificationsDelegate` и реализуйте метод `silentPushNotificationReceived`. Подробнее — в разделе [Обработка полученных push-уведомлений](#handle-received-push-notifications).
...
> 👍
>
> Для использования звуков вам нужно реализовать [богатые push-уведомления](#rich-push-notifications).
...
Чтобы отслеживать доставку push-уведомлений, добавьте **Notification Service Extension**, как описано в разделе о [богатых push-уведомлениях](#rich-push-notifications).
...
[Методы делегата приложения](#implement-application-delegate-methods), описанные выше, обеспечивают корректную обработку push-уведомлений, отправленных из Engagement. 
...
Метод `trackPushOpened` принимает полезную нагрузку в [формате Engagement](#payload-example). При необходимости вы также можете отправлять события `campaign` вручную для уведомлений с любым другим форматом полезной нагрузки.
