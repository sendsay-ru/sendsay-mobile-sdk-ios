...
> 👍
>
> SDK предоставляет функцию самопроверки настройки push, чтобы помочь разработчикам успешно настроить push-уведомления. Самопроверка попытается отследить push-токен, запросить у бэкенда CDP Sendsay отправить тихий push на устройство и проверить, готово ли приложение открывать push-уведомления.
>
> Чтобы включить проверку настройки, установите `Sendsay.shared.checkPushSetup = true` **до** [инициализации SDK](setup.md#инициализация-sdk):
...
По умолчанию SDK отслеживает токен push-уведомления только в том случае, если приложение авторизовано (если не включена [проверка настройки push](#шаг-2-настройка-sdk)). Обратитесь к [Тихие Push-уведомления](#silent-push-notifications) ниже, чтобы узнать, как отслеживать push-токен даже когда приложение не авторизовано.

#### Контрольный список: 
 - [ ] CDP Sendsay теперь должен иметь возможность отправлять push-уведомления на ваше устройство. Для инструкций обратитесь к руководству [Создание нового уведомления](https://docs.sendsay.ru/other-channels/mobile-push/how-to-create-mobile-push-campaign).
...
Для получения push-уведомлений приложение должно отследить push-токен в бэкенд CDP Sendsay. SDK делает это автоматически только в том случае, если отслеживание push-уведомлений включено и правильно реализовано, и приложение [авторизовано](#register-for-receiving-push-notifications) для получения push-уведомлений с предупреждениями.
...
#### Контрольный список:
 - [ ] Проверьте, что push-уведомления с изображениями и кнопками, отправленные из CDP Sendsay, правильно отображаются на вашем устройстве. Отслеживание доставки push должно работать.
...
Чтобы использовать стандартный звук для уведомления, введите `default` в качестве значения для `Media > Sound` в вашем сценарии push-уведомления в веб-приложении CDP Sendsay.
![Настройка звука для push-уведомления в CDP Sendsay](https://raw.githubusercontent.com/sendsay/sendsay-ios-sdk/main/Documentation/images/push-sound-config.png)
...
После того как настраиваемый звук размещен в вашем приложении, введите имя звукового файла в качестве значения для `Media > Sound` в вашем сценарии push-уведомления в веб-приложении CDP Sendsay. Убедитесь, что вы вводите точное имя файла (с учетом регистра) без расширения.
...
Вызов `SendsayNotificationService.process` в `didReceive` будет отслеживать доставку уведомления как событие `campaign` в CDP Sendsay.

### Получение токена push-уведомления вручную

Иногда вашему приложению может потребоваться получить текущий push-токен во время работы. Вы можете сделать это, используя метод `Sendsay.shared.trackPushToken`. Обратитесь к [Отслеживание](tracking.md#отслеживание-сеанса-вручную) для подробностей.
...
>
> При последующем вызове `Sendsay.shared.identifyCustomer` всегда используйте [hard ID](tracking.md#идентификация). Использование soft ID может непреднамеренно привести к тому, что токен push-уведомления будет связан с неправильным профилем клиента.
...
SDK обрабатывает только push-уведомления, отправленные с платформы CDP Sendsay. Если вы используете платформы, отличные от CDP Sendsay, для отправки push-уведомлений, вы должны реализовать часть логики обработки уведомлений самостоятельно. 

#### Условная обработка

[Реализация методов делегата приложения](#implement-application-delegate-methods) выше описывает методы делегата, необходимые для работы обработки push-уведомлений CDP Sendsay. Вы можете использовать метод `Sendsay.isSendsayNotification(userInfo:)` в реализациях делегата для проверки, поступает ли входящее уведомление от CDP Sendsay, и если нет, обработать уведомление, используя реализацию для другого источника уведомлений.

#### Ручное отслеживание
Вы можете полностью отключить отслеживание уведомлений и использовать методы `Sendsay.shared.trackPushToken` и `Sendsay.shared.trackPushOpened` для ручного отслеживания событий push-уведомлений. `trackPushOpened` ожидает [формат полезной нагрузки CDP Sendsay](#payload-example). Вы всегда можете отследить событие `campaign` вручную для любого формата полезной нагрузки.

> ❗️
>
> На поведение `trackPushReceived` и `trackClickedPush` может влиять функция согласия на отслеживание, которая в включенном режиме учитывает требование явного согласия на отслеживание. Подробнее читайте в документации [согласие на отслеживание](tracking-consent.md).

### Настраиваемые действия уведомлений в iOS 11 и ниже

Для поддержки кнопок действий в iOS 11 и ниже, которые можно настроить в веб-приложении CDP Sendsay, вы должны реализовать настраиваемые категории уведомлений, которые используются для подключения действий и заголовков кнопок. SDK предоставляет удобный фабричный метод для упрощения создания такой категории.

> ❗️
>
> Идентификатор категории, который вы указываете здесь, должен быть идентичен тому, который вы указываете в бэкенде CDP Sendsay.
