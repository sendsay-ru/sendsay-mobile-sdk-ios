...
public struct GdprTracking {
    public static func isTrackForced(_ url: String?) -> Bool {
        ...
        case "0" : return false
        default :
            Sendsay.logger.log(.error, message: "Action url contains force-track with incompatible value \(rawForceTrackValue)")
            return false
        }
    }
    ...
    public static func readTrackingConsentFlag(_ source: Any?) -> Bool {
        ...
        if let sourceFlag = source as? String {
            ...
            case "0":
                return false
            default:
                Sendsay.logger.log(.error, message: "HasConsentFlag with incompatible value \(sourceFlag)")
                return false
            }
        }
        ...
        Sendsay.logger.log(.error, message: "HasConsentFlag with incompatible value \(String(describing: source))")
        return false
    }
}
