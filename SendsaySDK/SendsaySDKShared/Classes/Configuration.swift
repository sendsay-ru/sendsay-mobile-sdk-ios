...
/// A configuration object used to configure Sendsay when initialising.
public struct Configuration: Codable, Equatable {
    public internal(set) var projectMapping: [EventType: [SendsayProject]]?
    ...
    /// should the push notification token be sent to Sendsay.
    ...
    public init(plistName: String) throws {
        ...
        throw SendsayError.configurationError("Configuration plist not found.")
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let projectToken = try container.decodeIfPresent(String.self, forKey: .projectToken) else {
            throw SendsayError.configurationError("No project token provided.")
        }
        self.projectToken = projectToken
        ...
        if let dictionary = try container.decodeIfPresent(
            Dictionary<String, [SendsayProject]>.self, forKey: .projectMapping) {
            var mapping: [EventType: [SendsayProject]] = [:]
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let mapping = projectMapping {
            let projectMappingWithStringKeys = Dictionary(
                uniqueKeysWithValues: mapping.map { (key: EventType, value: [SendsayProject]) in (key.rawValue, value) }
            )
...
extension Configuration {
    public func projects(for eventType: EventType) -> [SendsayProject] {
        var projects: [SendsayProject] = [mainProject]
    ...
    public var mainProject: SendsayProject {
        SendsayProject(baseUrl: baseUrl, projectToken: projectToken, authorization: authorization)
    }
    ...
    public var mutualSendsayProject: SendsayProject {
    ...
    private func loadCustomAuthProvider() throws -> AuthorizationProviderType? {
        let className = "SendsayAuthProvider"
        ...
        guard let asProviderClass = asNSObjectClass as? AuthorizationProviderType.Type else {
            Sendsay.logger.log(.error, message: "Class '\(className)' does not conform to AuthorizationProviderType")
            throw ConfigurationValidationError.advancedAuthInvalid(
...
extension Configuration: CustomStringConvertible {
    ...
    public var hostname: String {
        guard let components = URLComponents(string: baseUrl),
            ...
            Sendsay.logger.log(.warning, message: "Can't get URL components from baseUrl, check your baseUrl.")
            return baseUrl
        }
