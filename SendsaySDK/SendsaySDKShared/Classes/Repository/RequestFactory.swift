...
public struct RequestFactory {
    let sendsayProject: SendsayProject
    let route: Routes

    public init(sendsayProject: SendsayProject, route: Routes) {
        self.sendsayProject = sendsayProject
        self.route = route
    }

    public var path: String {
        let baseUrl = sendsayProject.baseUrl.last == "/" ?
...
public extension RequestFactory {
    func prepareRequest(
        ...
        switch sendsayProject.authorization {
        case .none: break
        ...
        if let parameters = parameters?.requestParameters {
            ...
            do {
                ...
                Sendsay.logger.log(.error,
                                   message: "Failed to serialise request body into JSON: \(error.localizedDescription)")
                Sendsay.logger.log(.verbose, message: "Request parameters: \(params)")
            }
        }

        // Log request if necessary
        if Sendsay.logger.logLevel == .verbose {
            Sendsay.logger.log(.verbose, message: "Created request: \n\(request.describe)")
        }
