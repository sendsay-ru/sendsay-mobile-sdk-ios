...
final class Coordinator {
    ...
    let tabbar: SendsayTabBarController?
    private let deeplinkManager = DeeplinkManager.manager

    init(navigationController: UINavigationController?) {
        self.navigationController = navigationController
        self.tabbar = UIStoryboard(name: "Main", bundle: nil)
            .instantiateViewController(withIdentifier: "tabbar") as? SendsayTabBarController
        self.tabbar?.tabBar.tintColor = .white
        self.tabbar?.tabBar.unselectedItemTintColor = .colorAccent
        // Delay presenting vc
