...
class ExampleInAppContentBlockCallback: InAppContentBlockCallbackType {
    ...
    func onMessageShown(placeholderId: String, contentBlock: SendsaySDK.InAppContentBlockResponse) {
        ...
        Sendsay.logger.log(.verbose, message: "Content block with HTML: \(htmlContent ?? "empty")")
        ...
        if let htmlContent,
           let normalizedHtml = HtmlNormalizer(htmlContent).normalize(normalizerConf).html {
            Sendsay.logger.log(.verbose, message: "Normalized HTML: \(normalizedHtml)")
        }
    }
    ...
    func onError(placeholderId: String, contentBlock: SendsaySDK.InAppContentBlockResponse?, errorMessage: String) {
        ...
        Sendsay.shared.trackInAppContentBlockErrorWithoutTrackingConsent(
    ...
    func onCloseClicked(placeholderId: String, contentBlock: SendsaySDK.InAppContentBlockResponse) {
        Sendsay.shared.trackInAppContentBlockCloseWithoutTrackingConsent(
            placeholderId: placeholderId,
            message: contentBlock
        )
    }
    ...
    func onActionClicked(placeholderId: String, contentBlock: SendsaySDK.InAppContentBlockResponse, action: SendsaySDK.InAppContentBlockAction) {
        Sendsay.logger.log(.verbose, message: "Handling In-app content block action \(action.url ?? "none")")
        ...
        if actionClickBounce {
            ...
            Sendsay.shared.trackInAppContentBlockClickWithoutTrackingConsent(
                placeholderId: placeholderId,
                action: action,
                message: contentBlock
            )
            ...
            case .close:
                Sendsay.logger.log(.error, message: "In-app content block close has to be handled elsewhere")
            case .unknown:
                Sendsay.logger.log(.error, message: "In-app content block unknown operation")
            }
        } else {
