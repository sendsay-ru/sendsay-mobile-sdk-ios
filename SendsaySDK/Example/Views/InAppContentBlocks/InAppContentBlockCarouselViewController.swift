...
class CustomCarouselCallback: DefaultContentBlockCarouselCallback {
    ...
    public func onMessageShown(placeholderId: String, contentBlock: SendsaySDK.InAppContentBlockResponse, index: Int, count: Int) {
        // space for custom implementation
    }

    public func onMessagesChanged(count: Int, messages: [SendsaySDK.InAppContentBlockResponse]) {
        // space for custom implementation
    }
    ...
    public func onHeightUpdate(placeholderId: String, height: CGFloat) {
        Sendsay.logger.log(.verbose, message: "Placeholder \(placeholderId) got new height: \(height)")
    }
}
