...
class InAppContentBlocksViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case 1, 3, 5:
            return Sendsay.shared.inAppContentBlocksManager?.getUsedInAppContentBlocks(placeholder: "example_list", indexPath: indexPath)?.height ?? 0
        default:
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 1, 3, 5:
            ...
            if let view = Sendsay.shared.inAppContentBlocksManager?.prepareInAppContentBlockView(placeholderId: "example_list", indexPath: indexPath) {
    ...
    override func viewDidLoad() {
        ...
        placeholder.contentReadyCompletion = { [weak self] contentLoaded in
            guard let self else {
                Sendsay.logger.log(.error, message: "In-app content block has been loaded but deattached from view controller")
                return
            }
            Sendsay.logger.log(.verbose, message: "In-app content block has been loaded: \(contentLoaded)")
            if contentLoaded {
