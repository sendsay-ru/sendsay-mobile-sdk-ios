...
final class SessionManager: SessionManagerType {
    ...
    func ensureSessionStarted(at timestamp: TimeInterval) {
        guard isAutomatic else { return }
        if !hasActiveSession {
            Sendsay.logger.log(.verbose, message: "Manually triggering session start")
            triggerSessionStart(at: timestamp)
        }
    }
    ...
    private func triggerSessionStart(at timestamp: TimeInterval) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "Session_start not tracked, SDK is stopping")
            return
        }
        // if previous session was not closed properly(background work was not run)
        if hasActiveSession {
            if !shouldResumeCurrentSession(at: timestamp) {
                Sendsay.logger.log(.verbose, message: "We're past session timeout, tracking previous session end.")
                if isAutomatic || manualSessionAutoClose {
                    triggerSessionEnd(at: timestamp)
                }
            } else {
                Sendsay.logger.log(.verbose, message: "Continuing current session as we're within session timeout.")
                return
            }
        }
        sessionStartTime = timestamp
        trackingDelegate?.trackSessionStart(at: sessionStartTime)
        Sendsay.logger.log(.verbose, message: Constants.SuccessMessages.sessionStart)
    }

    private func triggerSessionEnd(at timestamp: TimeInterval) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "Session_end not tracked, SDK is stopping")
            return
        }
        guard sessionStartTime != 0 else {
            Sendsay.logger.log(.error, message: "Can't end session as no session was started.")
            return
        }
        ...
        Sendsay.logger.log(.verbose, message: Constants.SuccessMessages.sessionEnd)
    }
}
