...
class TrackingManager {
    ...
    private var backgroundTask: UIBackgroundTaskIdentifier = UIBackgroundTaskIdentifier.invalid {
        didSet {
            if backgroundTask == UIBackgroundTaskIdentifier.invalid && backgroundWorkItem != nil {
                Sendsay.logger.log(.verbose, message: "Background task ended, stopping background work item.")
    ...
    private var backgroundWorkItem: DispatchWorkItem? {
        didSet {
            ...
            if backgroundWorkItem == nil && backgroundTask != UIBackgroundTaskIdentifier.invalid {
                Sendsay.logger.log(.verbose, message: "Stopping background task after work item done/cancelled.")
    ...
    deinit {
        Sendsay.logger.log(.verbose, message: "TrackingManager deallocated.")
    }
    ...
    internal func trackInstallEvent() {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "Install event not tracked, SDK is stopping.")
            return
        }
        ...
        guard userDefaults.bool(forKey: key) == false else {
            Sendsay.logger.log(.verbose, message: "Install event was already tracked, skipping.")
            return
        }
...
extension TrackingManager: TrackingManagerType {
    ...
    private func trackInternal(
        ...
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "track internal failed, Sendsay is stopped")
            return
        }
        ...
        if trackingAllowed {
            Sendsay.logger.log(.verbose, message: "Tracking event of type: \(type) with params \(data ?? [])")
        } else {
            Sendsay.logger.log(.verbose, message: "Processing event of type: \(type) with params \(data ?? []) with tracking \(trackingAllowed)")
        }
    ...
    func setAutomaticSessionTracking(automaticSessionTracking: Sendsay.AutomaticSessionTracking) {
    ...
    func trackDeliveredPushEvent(_ eventObject: EventTrackingObject) {
        ...
        let project = eventObject.sendsayProject
        ...
        do {
            ...
            Sendsay.logger.log(.error, message: "Error while tracking push opened event: \(error.localizedDescription)")
        }
    }
}
...
extension TrackingManager: SessionTrackingDelegate {
    func trackSessionStart(at timestamp: TimeInterval) {
        do {
            ...
            Sendsay.logger.log(.error, message: "Session start tracking error: \(error.localizedDescription)")
        }
    }
    ...
    func trackSessionEnd(at timestamp: TimeInterval, withDuration duration: TimeInterval) {
        ...
        do {
            ...
            Sendsay.logger.log(.error, message: "Session end tracking error: \(error.localizedDescription)")
        }
    }
}
...
extension TrackingManager {
    ...
    internal func applicationDidEnterBackgroundUnsafe() {
        ...
        Sendsay.logger.log(.verbose, message: "Started background task with delay \(delay)s.")
    }

    internal func createBackgroundWorkItem() -> DispatchWorkItem {
        let unsafeWork = { [weak self] in
            guard let `self` = self else { return }
            guard Sendsay.shared.isConfigured else { return }
        ...
        return DispatchWorkItem { Sendsay.shared.executeSafely { unsafeWork() } }
    }
}
...
extension TrackingManager {
    public func anonymize(sendsayProject: SendsayProject, projectMapping: [EventType: [SendsayProject]]?) throws {
        ...
        repository.configuration.switchProjects(mainProject: sendsayProject, projectMapping: projectMapping)
