...
final class FileCache: FileCacheType {
    ...
    static let inAppMessagesFolder = "sendsaysdk_files_cache"
    ...
    func hasFileData(at fileUrl: String) -> Bool {
        guard let directory = getCacheDirectoryURL() else {
            Sendsay.logger.log(.warning, message: "Unable to get file cache directory")
            return false
        }
        let fileUrl = directory.appendingPathComponent(FileUtils.getFileName(fileUrl: fileUrl))
        let exists = fileManager.fileExists(atPath: fileUrl.path)
        if !exists {
            Sendsay.logger.log(.verbose, message: "File \(fileUrl) not found in cache.")
        }
        return exists
    }
