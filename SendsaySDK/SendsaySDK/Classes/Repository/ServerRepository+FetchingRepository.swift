...
extension ServerRepository: FetchRepository {
    func fetchRecommendation<T: RecommendationUserData>(
        ...
        let router = RequestFactory(sendsayProject: configuration.mainProject, route: .customerAttributes)
    ...
    func fetchConsents(completion: @escaping (Result<ConsentsResponse>) -> Void) {
        let router = RequestFactory(sendsayProject: configuration.mainProject, route: .consents)
    ...
    func fetchInAppMessages(
        ...
        let router = RequestFactory(sendsayProject: configuration.mainProject, route: .inAppMessages)
    ...
    func fetchAppInbox(
        ...
        let router = RequestFactory(sendsayProject: configuration.mutualSendsayProject, route: .appInbox)
        let request = router.prepareRequest(
    ...
    func postReadFlagAppInbox(
        ...
        let router = RequestFactory(sendsayProject: configuration.mutualSendsayProject, route: .appInboxMarkRead)
        let request = router.prepareRequest(
    ...
    func getInAppContentBlocks(
        ...
        let router = RequestFactory(sendsayProject: configuration.mutualSendsayProject, route: .inAppContentBlocks)
        let request = router.prepareRequest()
        session
            .dataTask(with: request, completionHandler: router.handler(with: completion))
            .resume()
    ...
    func personalizedInAppContentBlocks(
        ...
        let router = RequestFactory(sendsayProject: configuration.mutualSendsayProject, route: .personalizedInAppContentBlocks)
        let request = router.prepareRequest(
    ...
    func getSegmentations(
        ...
        let router = RequestFactory(sendsayProject: configuration.mutualSendsayProject, route: .segmentation(cookie: cookie))
        let request = router.prepareRequest()
        session
            .dataTask(with: request, completionHandler: router.handler(with: completion))
            .resume()
    ...
    func getLinkIds(
        ...
        let router = RequestFactory(sendsayProject: configuration.mutualSendsayProject, route: .linkIds(cookie: cookie))
        let request = router.prepareRequest(
