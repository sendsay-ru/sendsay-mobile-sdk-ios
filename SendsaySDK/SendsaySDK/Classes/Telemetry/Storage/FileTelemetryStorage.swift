...
class FileTelemetryStorage: TelemetryStorage {
    let crashLogFolder = "sendsaysdk_telemetry_storage"
    let crashLogFilePrefix = "sendsaysdk_crashlog_"
    ...
    func saveCrashLog(_ log: CrashLog) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "saveCrashLog skipped, SDK is stopped")
            return
        }
        guard let jsonData = try? JSONEncoder().encode(log),
            let jsonString = String(data: jsonData, encoding: .utf8),
            let fileURL = getFileURL(log) else {
                Sendsay.logger.log(.warning, message: "Unable to serialize crash log")
                return
        }
    ...
    func getAllCrashLogs() -> [CrashLog] {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "getAllCrashLogs skipped, SDK is stopped")
            return []
        }
        guard let dir = makeCacheDirectory() else {
            return []
