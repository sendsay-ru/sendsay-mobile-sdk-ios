...
final class CrashManager {
    ...
    private let logsQueue = DispatchQueue(label: "com.sendsay.telemetry.crashmanager.logs")
    ...
    deinit {
        if let hookId = logHookId {
            Sendsay.logger.removeLogHook(with: hookId)
        }
    }
    ...
    func start() {
        ...
        isStarted = true
        logHookId = Sendsay.logger.addLogHook(self.reportLog(_:))
        uploadCrashLogs()
    ...
    func uncaughtExceptionHandler(_ exception: NSException) {
        ...
        Sendsay.logger.log(.error, message: "Handling uncaught exception")
    ...
    func uploadCaughtCrashLog(_ crashLog: CrashLog) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "uploadCaughtCrashLog skipped, SDK is stopped")
            return
        }
        upload.upload(crashLog: crashLog) { result in
            if !result {
                Sendsay.logger.log(.error, message: "Uploading crash log failed")
    ...
    func uploadCrashLogs() {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "uploadCrashLogs skipped, SDK is stopped")
            return
        }
        storage.getAllCrashLogs().forEach { crashLog in
            if crashLog.timestamp > Date().timeIntervalSince1970 - CrashManager.logRetention {
                upload.upload(crashLog: crashLog) { result in
                    if result {
                        Sendsay.logger.log(.verbose, message: "Successfully uploaded crash log")
