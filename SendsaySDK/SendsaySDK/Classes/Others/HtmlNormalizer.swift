...
// Copyright (c) 2022 Sendsay. All rights reserved.
//
...
public class HtmlNormalizer {
    ...
    private let closeActionCommandUrlPrefix = "https://sendsay.com/close_action_"
    ...
    public func normalize(_ config: HtmlNormalizerConfig? = nil) -> NormalizedResult {
        ...
        do {
            ...
            Sendsay.logger.log(.error, message: "[HTML] Html has not been processed due to error \(error)")
            result.valid = false
        }
        return result
    }
    ...
    private func exportHtml() -> String? {
        guard let document = document else {
            Sendsay.logger.log(.warning, message: "[HTML] Document has not been initialized, no HTML to export")
            return nil
        }
        do {
            return try document.html()
        } catch let error {
            Sendsay.logger.log(.error, message: "[HTML] Output cannot be exported: \(error)")
            return nil
        }
    }
    ...
    private func ensureActionButtons() throws -> [ActionInfo] {
        ...
        guard let document = document else {
            Sendsay.logger.log(.warning, message: "[HTML] Document has not been initialized, no Action buttons")
            return []
        }
    ...
    private func collectAnchorLinkButtons(_ document: Document) throws -> [ActionInfo] {
        ...
        for actionButton in anchorlinkButtons.array() {
            ...
            if targetAction.isEmpty {
                Sendsay.logger.log(.error, message: "[HTML] Action button found but with empty action")
                continue
            }
    ...
    private func collectDataLinkButtons(_ document: Document) throws -> [ActionInfo] {
        ...
        for actionButton in datalinkButtons.array() {
            ...
            if try actionButton.iS(anchorTagSelector) {
                ...
                Sendsay.logger.log(.verbose, message: "[HTML] Wrapping Action button with a-href")
    ...
    private func detectCloseButton(_ ensureCloseButton: Bool) throws -> [ActionInfo] {
        guard let document = document,
            ...
            throw SendsayError.unknownError("Action close cannot be ensured")
        }
        ...
        for closeButton in closeButtons {
            ...
            if try closeButtonParent.iS(anchorTagSelector) == false {
                Sendsay.logger.log(.verbose, message: "[HTML] Wrapping Close button with a-href")
    ...
    private func makeResourcesToBeOffline() throws {
        guard document != nil else {
            Sendsay.logger.log(.warning, message: "[HTML] Document has not been initialized, no Image to process")
            return
        }
    ...
    private func downloadOnlineResources(_ styleSource: String) -> String {
        ...
        for statement in onlineStatements {
            ...
            default:
                dataBase64 = nil
                Sendsay.logger.log(.error, message: "Unsupported mime type \(statement.mimeType)")
            }
            guard let dataBase64 = dataBase64 else {
                Sendsay.logger.log(.error, message: "Unable to make offline resource \(statement.url)")
                continue
            }
    ...
    private func makeStylesheetsToBeOffline() throws {
        ...
        for styledTag in styleTags {
            ...
            do {
                ...
                Sendsay.logger.log(.error, message: "[HTML] Element <style> not updated: \(error.localizedDescription)")
                throw error
            }
        }
    }
    ...
    public func collectImages() -> [String] {
        ...
        do {
            ...
            Sendsay.logger.log(.warning, message: "[HTML] Failure while reading image source: \(error)")
        }
        // style tags
        do {
            ...
            Sendsay.logger.log(.warning, message: "[HTML] Failure while reading style tag source: \(error)")
        }
        // style attributes
        do {
            ...
            Sendsay.logger.log(.warning, message: "[HTML] Failure while reading style attribute source: \(error)")
        }
        // end
        return onlineUrls
    }
    ...
    private func removeElements(_ selector: String) throws {
        guard let document = document else {
            Sendsay.logger.log(.warning, message: "[HTML] Document has not been initialized, no elements removing")
            return
        }
    ...
    private func removeAttributes(_ attribute: String, skipTag: String? = nil) throws {
        guard let document = document else {
            Sendsay.logger.log(.warning, message: "[HTML] Document has not been initialized, no Attributes removing")
            return
        }
