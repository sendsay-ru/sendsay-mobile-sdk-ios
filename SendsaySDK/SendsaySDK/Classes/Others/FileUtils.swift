...
struct FileUtils {
    ...
    public static func tryDownloadFile(_ fileSource: String?) -> Data? {
        guard let fileSource = fileSource,
            ...
            Sendsay.logger.log(.error, message: "File cannot be downloaded \(fileSource ?? "<is nil>")")
            return nil
        }
        ...
        case .success:
            ...
            break
        case .timedOut:
            Sendsay.logger.log(.warning, message: "File \(fileSource) may be too large or slow connection - aborting")
            dataTask.cancel()
        }
        return fileData
    }
