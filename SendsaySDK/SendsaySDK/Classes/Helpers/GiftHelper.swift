...
extension UIImage {
    public class func gifImageWithData(_ data: Data) -> UIImage? {
        guard let source = CGImageSourceCreateWithData(data as CFData, nil), isGif(data: data) else {
            Sendsay.logger.log(.error, message: "Invalid image data")
            return nil
        }
    ...
    public class func gifImageWithURL(_ gifUrl: String) -> UIImage? {
        guard let url: URL = URL(string: gifUrl) else {
            Sendsay.logger.log(.error, message: "[GiftHelper] Invalid image url \(gifUrl)")
            return nil
        }
        guard isGif(imageUrl: url), let imageData = try? Data(contentsOf: url) else {
            Sendsay.logger.log(.verbose, message: "[GiftHelper] Image \(gifUrl) is not a gif")
            return nil
        }
