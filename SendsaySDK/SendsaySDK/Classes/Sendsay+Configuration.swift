...
public extension Sendsay {
    struct ProjectSettings {
        public let projectToken: String
        public let authorization: Authorization
        public let baseUrl: String
        public let projectMapping: [EventType: [SendsayProject]]?

        public init(
            projectToken: String,
            authorization: Authorization,
            baseUrl: String? = nil,
            projectMapping: [EventType: [SendsayProject]]? = nil
        ) {
...
public extension SendsayInternal {
    ...
    private func invokeSdkInitSafely(
        ...
        Sendsay.logger.log(.verbose, message: "Request of SDK initialization registered")
        if onInitSucceededCallBack != nil {
            asyncSdkInitialisationQueue.addOperation {
                self.sdkInitialisationBlockQueue.sync {
                    ...
                    do {
                        Sendsay.logger.log(.verbose, message: "SDK init starts asynchronously")
                        try initBlock()
                        self.afterInit.setStatus(status: .configured)
                        Sendsay.logger.log(.verbose, message: "SDK init ends asynchronously")
                        onMain {
                            self.onInitSucceededCallBack?()
                        }
                    } catch {
                        Sendsay.logger.log(.error, message: "Can't create configuration: \(error.localizedDescription)")
                    }
                }
            }
            return
        ...
        self.sdkInitialisationBlockQueue.sync {
            ...
            do {
                Sendsay.logger.log(.verbose, message: "SDK init starts synchronously")
                try initBlock()
                self.afterInit.setStatus(status: .configured)
                Sendsay.logger.log(.verbose, message: "SDK init ends synchronously")
            } catch {
                Sendsay.logger.log(.error, message: "Can't create configuration: \(error.localizedDescription)")
            }
        }
    ...
    func configure(
        ...
        automaticSessionTracking: Sendsay.AutomaticSessionTracking = .enabled(),
        defaultProperties: [String: JSONConvertible]? = nil,
        inAppContentBlocksPlaceholders: [String]? = nil,
        flushingSetup: Sendsay.FlushingSetup = Sendsay.FlushingSetup.default,
    ...
    func configure(
        _ projectSettings: Sendsay.ProjectSettings,
        pushNotificationTracking: Sendsay.PushNotificationTracking,
        automaticSessionTracking: Sendsay.AutomaticSessionTracking = .enabled(),
        defaultProperties: [String: JSONConvertible]? = nil,
        inAppContentBlocksPlaceholders: [String]? = nil,
        flushingSetup: Sendsay.FlushingSetup = Sendsay.FlushingSetup.default,
    ...
    ///   - authorization: The authorization type used to authenticate with some Sendsay endpoints.
    ///   - baseUrl: Base URL used for the project, for example if you use a custom domain with your Sendsay setup.
    ...
    /// Initialize the configuration with a plist file containing the keys for the SendsaySDK.
    ///
    /// - Parameters:
