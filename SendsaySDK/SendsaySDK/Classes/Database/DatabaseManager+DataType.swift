...
extension DatabaseManager {
    static func processObject(_ object: NSObject) -> JSONValue? {
        ...
        Sendsay.logger.log(.warning, message: "Skipping object of unsupported type: \(object.self)")
        return nil
    }
    ...
    static func processArray(_ array: NSArray) -> JSONValue {
        ...
        for element in array {
            guard let object = element as? NSObject else {
                Sendsay.logger.log(.warning, message: "Skipping array element of unsupported type: \(element.self)")
                continue
            }
            ...
            if let nested = object as? NSDictionary {
                ...
                Sendsay.logger.log(.warning, message: "Skipping array element of unsupported type: \(object.self)")
                continue
            }
        }
    ...
    static func transformPrimitiveType(_ object: NSObject) -> JSONValue? {
        ...
        case is NSString:
            guard let string = object as? NSString else {
                Sendsay.logger.log(.error, message: "Failed to convert NSObject to NSString for: \(object).")
                return nil
            }
        ...
        case is NSNumber:
            guard let number = object as? NSNumber else {
                Sendsay.logger.log(.error, message: "Failed to convert NSObject to NSNumber for: \(object).")
                return nil
            }
        ...
        default:
            Sendsay.logger.log(.warning, message: "Skipping unsupported property value: \(object).")
            return nil
        }
    }
}
