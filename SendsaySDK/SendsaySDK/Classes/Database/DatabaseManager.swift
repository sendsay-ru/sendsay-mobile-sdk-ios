...
/// Persisted data will be used to interact with the Sendsay API.
class DatabaseManager {
    ...
    internal init(persistentStoreDescriptions: [NSPersistentStoreDescription]? = nil) throws {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "Init database, SDK is stopped")
        ...
        if let error = loadError {
            Sendsay.logger.log(.error, message: "Unresolved error \(error.localizedDescription).")
        ...
        // Initialise customer
        _ = currentCustomer
        Sendsay.logger.log(.verbose, message: "Database initialised with customer:\n\(currentCustomer)")
    }
}
    ...
    private var currentCustomerManagedObject: Customer {
        return context.performAndWait {
            do {
                ...
                Sendsay.logger.log(.warning, message: "No customer found saved in database, will create. \(error)")
            }
    ...
    private func makeNewCustomerInternal() -> Customer {
        return context.performAndWait {
            ...
            do {
                ...
                Sendsay.logger.log(.verbose, message: "New customer created with UUID: \(customer.uuid)")
    ...
    var customers: [CustomerThreadSafe] {
        return context.performAndWait {
            do {
                ...
                Sendsay.logger.log(.warning, message: "Error while fetching all customers. \(error)")
                return []
            }
        }
    }
...
extension DatabaseManager: DatabaseManagerType {
        ...
        if Sendsay.isBeingTested {
            ...
            context.performAndWait { [weak self] in
                ...
                do {
                    ...
                    Sendsay.logger.log(.error, message: "removeAllEvents failed: \(error)")
                }
            }
        }
    }
    ...
    func updateEvent(withId id: NSManagedObjectID, withData data: DataType) throws {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "Event \(data) has not been tracked, SDK is stopping")
            return
        }
        try context.performAndWait {
            ...
            Sendsay.logger.log(.verbose, message: "going to modify event with id \(event.objectID)")
            try saveContext(context)
        }
    }
    ...
    ///     - `timestamp`
    ///     - `eventType`
    func trackEvent(with data: [DataType], into project: SendsayProject) throws {
        guard !IntegrationManager.shared.isStopped else {
        ...
        try context.performAndWait {
            ...
            if data.customerIds.isEmpty {
                Sendsay.logger.log(.error, message: "Event track with no customer IDs occured, fallback to current")
            }
    ...
    func identifyCustomer(with data: [DataType], into project: SendsayProject) throws {
        try context.performAndWait {
    ...
    private func loadTrackingCustomer(context: NSManagedObjectContext, with data: [DataType], into project: SendsayProject) {
        ...
        if data.customerIds.isEmpty {
            Sendsay.logger.log(.error, message: "IdentifyCustomer event with no customer IDs occured, fallback to current")
        }
