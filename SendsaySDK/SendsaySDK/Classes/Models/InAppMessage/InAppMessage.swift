...
public struct InAppMessage: Codable, Equatable {
    ...
    func applyDateFilter(date: Date) -> Bool {
        ...
        if let start = dateFilter.startDate, start > date {
            Sendsay.logger.log(.verbose, message: "Message '\(self.name)' outside of date range.")
            return false
        }
    ...
    func applyEventFilter(event: [DataType]) -> Bool {
        ...
        eventTypes.forEach { eventType in
            ...
            do {
                ...
                Sendsay.logger.log(.error, message: "Error applying in-app message event filter \(error)")
            }
        }
        if !passed {
    ...
    func applyFrequencyFilter(displayState: InAppMessageDisplayStatus, sessionStart: Date) -> Bool {
        ...
        case .some(.onlyOnce):
            ...
            if !shouldDisplay {
                Sendsay.logger.log(.verbose, message: "Message '\(self.name)' already displayed.")
            }
            return shouldDisplay
        case .some(.oncePerVisit):
            let shouldDisplay = displayState.displayed ?? Date(timeIntervalSince1970: 0) < sessionStart
            if !shouldDisplay {
                Sendsay.logger.log(.verbose, message: "Message '\(self.name)' already displayed this session.")
            }
            return shouldDisplay
        case .some(.untilVisitorInteracts):
            let shouldDisplay = displayState.interacted == nil
            if !shouldDisplay {
                Sendsay.logger.log(.verbose, message: "Message '\(self.name)' already interacted with.")
            }
            return shouldDisplay
        case .none:
            Sendsay.logger.log(.warning, message: "Unknown in-app message frequency.")
            return true
        }
    }
