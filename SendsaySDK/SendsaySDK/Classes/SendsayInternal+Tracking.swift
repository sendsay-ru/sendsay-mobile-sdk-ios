//
//  SendsayInternal+Tracking.swift
//  SendsaySDK
//
...
extension SendsayInternal {
    ...
    public func identifyCustomer(customerIds: [String: String]?,
        ...
        executeSafelyWithDependencies { dependencies in
            ...
            if var ids = customerIds {
                ...
                if ids["cookie"] != nil {
                    Sendsay.logger.log(.warning, message: """
    ...
    /// Tracks the push notification token to Sendsay API with struct.
    ///
    ...
    /// Tracks the push notification token to Sendsay API with string.
    ///
    ...
    /// Tracks the push notification clicked event to Sendsay API.
    ...
    public func trackPushReceived(content: UNNotificationContent) {
        guard let userInfo = readUserInfo(content) else {
            Sendsay.logger.log(.error, message: " No user info object from notification.")
            return
        }
    ...
    func trackCampaignData(data: CampaignData, timestamp: Double?) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "Campaign event not tracked, SDK is stopping")
            return
        }
        Sendsay.logger.log(.verbose, message: "Tracking campaign data: \(data.description)")
        executeSafelyWithDependencies { dependencies in
            // Create initial data
            guard dependencies.configuration.authorization != Authorization.none else {
                throw SendsayError.authorizationInsufficient
            ...
            if dependencies.configuration.automaticSessionTracking {
                ...
                if try dependencies.trackingManager.hasPendingEvent(ofType: Constants.EventTypes.sessionStart,
                    ...
                    Sendsay.logger.log(.verbose, message: "Amending session start event with campaign data")
    ...
    public func handlePushNotificationOpened(userInfo: [AnyHashable: Any], actionIdentifier: String? = nil) {
        guard Sendsay.isSendsayNotification(userInfo: userInfo) else {
            Sendsay.logger.log(.verbose, message: "Skipping non-Sendsay notification")
            return
        }
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.verbose, message: "Skipping notification, SDK is stopping")
            return
        }
        executeSafelyWithDependencies { dependencies in
    ...
    // MARK: Flushing
    /// This method can be used to manually flush all available data to Sendsay.
    ...
    public func anonymize() {
        Sendsay.logger.log(.verbose, message: "Basic anonymisation requested")
        executeSafelyWithDependencies { dependencies in
            self.anonymize(
                sendsayProject: dependencies.configuration.mainProject,
    ...
    /// Anonymizes the user and starts tracking as if the app was just installed.
    /// All customer identification (including cookie) will be permanently deleted.
    /// Switches tracking into provided sendsayProject
    public func anonymize(
        sendsayProject: SendsayProject,
        projectMapping: [EventType: [SendsayProject]]?
    ) {
        Sendsay.logger.log(.verbose, message: "Anonymisation requested with \(sendsayProject) and \(String(describing: projectMapping))")
        executeSafelyWithDependencies { dependencies in
            Sendsay.logger.log(.verbose, message: "Anonymisation request proceeding")
            if dependencies.configuration.automaticSessionTracking {
