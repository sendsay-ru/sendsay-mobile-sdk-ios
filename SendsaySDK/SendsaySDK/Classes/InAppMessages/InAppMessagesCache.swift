...
final class InAppMessagesCache: InAppMessagesCacheType {
    static let inAppMessagesFolder = "sendsaysdk_in_app_messages"
    ...
    func saveInAppMessages(inAppMessages: [InAppMessage]) {
        ...
        guard let jsonData = try? JSONEncoder().encode(inAppMessages),
            ...
            let directory = getCacheDirectoryURL() else {
                Sendsay.logger.log(.warning, message: "Unable to serialize in-app messages data.")
                return
        }
        do {
            ...
            Sendsay.logger.log(.warning, message: "Saving in-app messages to file failed.")
        }
    }
    ...
    func hasImageData(at imageUrl: String) -> Bool {
        guard let directory = getCacheDirectoryURL() else {
            Sendsay.logger.log(.warning, message: "Unable to get in-app message image cache directory")
            return false
        }
        let fileUrl = directory.appendingPathComponent(FileUtils.getFileName(fileUrl: imageUrl))
        let exists = fileManager.fileExists(atPath: fileUrl.path)
        if !exists {
            Sendsay.logger.log(.verbose, message: "In-app message image \(imageUrl) not found in cache.")
        }
        return exists
    }
    ...
    func saveImageData(at imageUrl: String, data: Data) {
        ...
        do {
            ...
            Sendsay.logger.log(.error, message: "In-app message image \(imageUrl) failed to be stored: \(error)")
        }
    }
