...
final class InAppMessagePresenter: InAppMessagePresenterType {
            ...
            Sendsay.shared.executeSafely {
                onMain {
                    guard !self.presenting else {
                        Sendsay.logger.log(.verbose, message: "Already presenting in-app message.")
                    ...
                    if let imageData = imageData {
                        if let gifImage = UIImage.gifImageWithData(imageData) {
                            ...
                            Sendsay.logger.log(.error, message: "Unable to create in-app message image")
                    ...
                    guard let viewController = InAppMessagePresenter.getTopViewController(window: self.window) else {
                        Sendsay.logger.log(.error, message: "Unable to present in-app message - no view controller")
                    ...
                    do {
                        ...
                        Sendsay.logger.log(.verbose, message: "In-app message presented.")
                        ...
                        Sendsay.logger.log(.error, message: "Unable to present in-app message \(error)")
        ...
        case .modal, .fullscreen:
            ...
            if var payload {
                ...
                guard let image = image else {
                    Sendsay.logger.log(.error, message: "In-app message type \(messageType) requires image!")
    ...
    func createImage(imageData: Data, maxDimensionInPixels: Int) -> UIImage? {
        ...
        guard let imageSource = CGImageSourceCreateWithData(imageData as CFData, imageSourceOptions) else {
            Sendsay.logger.log(.error, message: "Unable create image for in-app message - image source failed")
            return nil
        }
        ...
        guard let downsampledCGImage = CGImageSourceCreateThumbnailAtIndex(imageSource, 0, downsampleOptions) else {
            Sendsay.logger.log(.error, message: "Unable create image for in-app message - downsampling failed")
            return nil
        }
