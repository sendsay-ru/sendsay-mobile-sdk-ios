...
final class InAppMessagesManager: InAppMessagesManagerType, @unchecked Sendable {
    ...
    func anonymize() {
        ...
        if let cookie = Sendsay.shared.trackingManager?.customerIds {
            Task { [weak self] in
    ...
    private var pickPendingMessage: InAppMessage? {
        guard let currentCustomerIds = Sendsay.shared.trackingManager?.customerIds, !presenter.presenting else {
            Sendsay.logger.log(
                .verbose,
                message: "[InApp] Pick pending messages faield due to customer id: \(Sendsay.shared.trackingManager?.customerIds ?? [:]) or presenter.presenting: \(presenter.presenting)"
            )
            return nil
        }
        Sendsay.logger.log(
    ...
    private func showInAppMessage(
        ...
        await withCheckedContinuation { [weak self] continuation in
            ...
            Sendsay.logger.log(.verbose, message: "[InApp] Attempting to show in-app message '\(message.name)'")
            var imageData: Data?
            ...
            self.presenter.presentInAppMessage(
                ...
                actionCallback: { button in
                    ...
                    if Sendsay.shared.inAppMessagesDelegate.trackActions {
                    ...
                    Sendsay.shared.inAppMessagesDelegate.inAppMessageClickAction(
                        message: message,
                    ...
                    if !Sendsay.shared.inAppMessagesDelegate.overrideDefaultBehavior {
                ...
                dismissCallback: { isUserInteraction, cancelButtonPayload in
                    ...
                    Sendsay.shared.inAppMessagesDelegate.inAppMessageCloseAction(
                        message: message,
                        button: cancelButton,
    ...
    private func trackInAppMessageError(
        ...
        Sendsay.shared.inAppMessagesDelegate.inAppMessageError(message: message, errorMessage: error)
    }
    ...
    private func checkPendingRequests() -> [DataType] {
        ...
        Sendsay.logger.log(.warning, message: "[InApp] No more pending requests")
        return []
    }
    ...
    func fetchInAppMessages(for event: [DataType], completion: EmptyBlock? = nil) {
        ...
        repository.fetchInAppMessages(for: event.customerIds) { [weak self] result in
            ...
            guard case let .success(response) = result,
                    let self,
                    var currentCustomerIds = Sendsay.shared.trackingManager?.customerIds
            else {
                Sendsay.logger.log(
                    .verbose,
                    message: "[InApp] fetchInAppMessages failed '\(result)', current customer: '\(Sendsay.shared.trackingManager?.customerIds ?? [:])'"
                )
                completion?()
                return
            }
    ...
    private func isFlushDone() async -> Bool {
        await withCheckedContinuation { continuation in
            switch Sendsay.shared.flushingMode {
            case .immediate:
                Sendsay.shared.flushingManager?.flushData()
                continuation.resume(returning: true)
            default:
                continuation.resume(returning: true)
            }
    ...
    internal func startIdentifyCustomerFlow(
        ...
        guard Sendsay.shared.isAppForeground else {
            Sendsay.logger.log(
                .verbose,
        ...
        case isFromIdentifyCustomer:
            ...
            guard Sendsay.shared.isAppForeground else { return }
            do {
                ...
                if var message = self.pickPendingMessage {
                    ...
                    if !self.presenter.presenting &&
                        event.customerIds.compareWith(
                            other: Sendsay.shared.trackingManager?.customerIds ?? [:]
                    ) {
                        Sendsay.logger.log(
                            .verbose,
                            message: """
