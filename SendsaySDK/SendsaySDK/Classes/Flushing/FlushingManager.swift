...
class FlushingManager: FlushingManagerType {
    ...
    private let reachability: SendsayReachability
    ...
    public var flushingMode: FlushingMode = .immediate {
        didSet {
            Sendsay.logger.log(.verbose, message: "Flushing mode updated to: \(flushingMode).")
    ...
    init(
        ...
        // Start reachability
        guard let reachability = SendsayReachability(hostname: repository.configuration.hostname) else {
    ...
    func flushData(isFromIdentify: Bool = false, completion: ((FlushResult) -> Void)?) {
        guard !IntegrationManager.shared.isStopped else {
            stopPeriodicFlushTimer()
            Sendsay.logger.log(.error, message: "Flushing has been denied, SDK is stopping")
            return
        }
        do {
            ...
            guard !isFlushingData else {
                Sendsay.logger.log(.warning, message: "Data flushing in progress, ignoring another flush call.")
            ...
            guard reachability.connection != .none else {
                Sendsay.logger.log(.warning, message: "Connection issues when flushing data, not flushing.")
