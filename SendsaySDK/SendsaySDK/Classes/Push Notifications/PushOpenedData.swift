...
struct PushOpenedData {
    ...
    let actionType: SendsayNotificationActionType
...
extension PushOpenedData: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        actionType = try data.decode(SendsayNotificationActionType.self, forKey: .actionType)
    ...
    public static func deserialize(from data: Data) -> PushOpenedData? {
        do {
            ...
            Sendsay.logger.log(.error, message: "Decoding push opened data failed: \(error.localizedDescription)")
        }
        return nil
    }
