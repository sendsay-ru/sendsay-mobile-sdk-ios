...
final class PushNotificationManager: NSObject, PushNotificationManagerType {
    ...
    init(
        ...
        // so we don't track push token to sendsay without permission to show
    ...
    func handlePushOpenedUnsafe(pushOpenedData: PushOpenedData) {
        ...
        if pushOpenedData.silent {
            ...
            if (pushOpenedData.considerConsent &&
                ...
                Sendsay.logger.log(.verbose, message: "Campaign data for delivered notification are not tracked because consent is not given")
            } else {
                // save campaign to be added to session start
                Sendsay.shared.trackCampaignData(data: pushOpenedData.campaignData, timestamp: nil)
            }
    ...
    func processStoredPushOpens() {
        ...
        guard let array = userDefaults?.array(forKey: Constants.General.openedPushUserDefaultsKey) else {
            Sendsay.logger.log(.verbose, message: "No opened push to track present in UserDefaults.")
            return
        }

        guard let dataArray = array as? [Data] else {
            Sendsay.logger.log(.warning, message: "Opened push data present in shared group but incorrect type.")
            return
        }

        for data in dataArray {
            guard let pushOpenedData = PushOpenedData.deserialize(from: data) else {
                Sendsay.logger.log(.warning, message: "Cannot deserialize stored opened push data.")
                continue
            }
            Sendsay.logger.log(.verbose, message: "Handling saved opened push notification.")
            handlePushOpenedUnsafe(pushOpenedData: pushOpenedData)
        }
    ...
    internal func checkForDeliveredPushMessages() {
        guard let appGroup = appGroup else {
            Sendsay.logger.log(.verbose, message: "No app group was setup, push delivered tracking is disabled.")
            return
        }
    ...
    internal func trackDeliveredPushMessages(_ source: UserDefaults) {
        guard let array = source.array(forKey: Constants.General.deliveredPushUserDefaultsKey) else {
            Sendsay.logger.log(.verbose, message: "No delivered push to track present in shared app group.")
            return
        }
        guard let dataArray = array as? [Data] else {
            Sendsay.logger.log(.warning, message: "Delivered push data present in shared group but incorrect type.")
            return
        }
    ...
    internal func trackDeliveredPushEvents(_ source: UserDefaults) {
        guard let array = source.array(forKey: Constants.General.deliveredPushEventUserDefaultsKey) else {
            Sendsay.logger.log(.verbose, message: "No delivered push events to track in shared app group.")
            return
        }
        guard let dataArray = array as? [Data] else {
            Sendsay.logger.log(.warning, message: "Delivered push events present in shared group but incorrect type.")
            return
        }
    ...
    private func trackCurrentPushToken(isAuthorized authorized: Bool) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "trackCurrentPushToken failed, Sendsay is stopped")
            return
        }
        do {
            ...
            Sendsay.logger.log(.error, message: "Error tracking current push token. \(error.localizedDescription)")
        }
    }
...
extension PushNotificationManager {
    func applicationDidBecomeActiveUnsafe() {
        checkForDeliveredPushMessages()
        // we don't have to check for opened pushes here, Sendsay SDK was initialized so it will be tracked directly
        verifyPushStatusAndTrackPushToken()
    }
}
