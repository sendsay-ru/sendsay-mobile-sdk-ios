...
final class PushNotificationSelfCheck {
        ...
        DispatchQueue.main.async {
            ...
            guard class_getInstanceMethod(type(of: appDelegate), Selectors.pushToken) != nil else {
                self.showResult(
                    ...
                    " Use SendsayAppDelegate helper or your own implementation " +
                    "with a call to Sendsay.shared.trackPushToken. "
                )
                return
            }
            ...
            guard class_getInstanceMethod(type(of: appDelegate), Selectors.receive) != nil else {
                self.showResult(
                    ...
                    " Use SendsayAppDelegate helper or your own implementation " +
                    "with a call to Sendsay.shared.handlePushNotificationOpened. "
                )
                return
            }
            ...
            guard UNUserNotificationCenter.current().delegate != nil else {
                self.showResult(
                    ...
                    " Use SendsayAppDelegate helper or your own implementation " +
                    "with call to Sendsay.shared.handlePushNotificationOpened."
                )
                return
            }
            completion()
    ...
    func waitForPushToken(delay: TimeInterval, retries: Int, completion: @escaping (String) -> Void) {
        ...
        if let pushToken = trackingManager.customerPushToken {
            if flushingManager.hasPendingData() {
                ...
                // we need to allow some time for Sendsay servers to process the push token
        ...
        repository.requestSelfCheckPush(
            ...
            completion: { result in
                guard result.error == nil else {
                    self.showResult(
                        step: 5,
                        message: "Unable to request push notification from sendsay server. " +
                        "Check your setup in Sendsay settings."
                    )
                    return
                }
                completion()
    ...
    func waitForSelfCheckPush(delay: TimeInterval, retries: Int, completion: @escaping () -> Void) {
        ...
        guard retries > 0 else {
            self.showResult(
                step: 6,
                message: "Unable to receive push notification from sendsay server. " +
                "Check your setup in Sendsay settings. " +
                "Make sure you're calling Sendsay.shared.handlePushNotificationOpened"
            )
            return
        }
    ...
    func showResult(step: Int, message: String) {
        ...
        let messageWithDetails = "\(message) \n\nSelf-check only runs in debug builds.\n" +
            "To disable it, set Sendsay.shared.checkPushSetup = false"
        Sendsay.logger.log(
