...
internal enum PushSelectorMapping {
    ...
    internal enum Swizzled {
        static let registration = #selector(
            UIResponder.sendsayApplicationSwizzle(_:didRegisterPushToken:)
        )

        static let centerDelegateReceive = #selector(
            NSObject.sendsayUserNotificationCenter(_:newDidReceive:withCompletionHandler:)
        )

        static let applicationReceive = #selector(
            UIResponder.sendsayApplication(_:newDidReceiveRemoteNotification:fetchCompletionHandler:)
        )
    }
