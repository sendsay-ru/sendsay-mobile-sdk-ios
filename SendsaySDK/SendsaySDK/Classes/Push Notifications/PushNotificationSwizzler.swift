...
final class PushNotificationSwizzler {
    ...
    /// This functions swizzles the token registration method to intercept the token and submit it to Sendsay.
    ...
    /// the `handlePushOpened` function with the payload so that the event can be tracked to Sendsay.
    ///
    ...
    private func swizzleNotificationReceived() {
        guard uiApplicationDelegating != nil,
              let appDelegate = uiApplicationDelegating.delegate else {
            Sendsay.logger.log(.error, message: "Critical error, no app delegate class available.")
            return
        }
