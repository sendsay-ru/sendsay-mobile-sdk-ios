...
open class AppInboxDetailViewController: UIViewController, WKUIDelegate {
    ...
    private var inAppContentBlocksManager: InAppContentBlocksManagerType? = Sendsay.shared.inAppContentBlocksManager
        ...
        actionManager = WebActionManager(
            onActionCallback: { [weak self] action in
                guard let self = self,
                    ...
                    Sendsay.logger.log(.error, message: "AppInbox action \(action.actionUrl) called but no action or message is provided")
                    return
                }
    ...
    private func applyDataToView() {
        guard
            ...
            Sendsay.logger.log(.warning, message: "Unsupported AppInbox type \(data?.type ?? "nil") to be shown")
            return
        }
        hideContainers()
        switch dataType {
        ...
        case "push":
            showPushMessage()
        default:
            Sendsay.logger.log(.error, message: "Unsupported AppInbox type \(dataType) to be shown")
            return
        }
    }
            ...
            if let imageUrl = self.data?.content?.imageUrl {
                if let image = UIImage.gifImageWithURL(imageUrl) {
                    ...
                    guard let imageSource = ImageUtils.tryDownloadImage(imageUrl),
                        ...
                        Sendsay.logger.log(.error, message: "Image cannot be shown")
                        return
                    }
                    onMain {
                        self.messageImage.image = image
    ...
    private func showHtmlMessage() {
        ...
        DispatchQueue.global(qos: .background).async { [weak self] in
            guard let selfWhileAsync = self else {
                Sendsay.logger.log(.error, message: "Showing a HTML AppInbox stops")
                return
            }
            ...
            guard
                ...
                Sendsay.logger.log(.error, message: "AppInbox message contains invalid HTML")
                return
            }
    ...
    @objc func invokeMainAction() {
        guard let action = mainAction,
            let message = data else {
                Sendsay.logger.log(.error, message: "AppInbox main action called but no action or message is provided")
                return
        }
    ...
    @objc func invokeActionForIndex(_ sender: UIButton) {
        ...
        guard let action = action,
            let message = data else {
                Sendsay.logger.log(.error, message: "AppInbox action \(index) called but no action or message is provided")
                return
        }
        invokeActionInternally(action, message)
    }
    ...
    private func invokeActionInternally(_ action: MessageItemAction, _ message: MessageItem) {
        Sendsay.shared.trackAppInboxClick(action: action, message: message)
        switch action.type {
        case .browser:
        ...
        default:
            Sendsay.logger.log(.warning, message: "No AppInbox action for type \(action.type.rawValue)")
        }
    }

    func openBrowserAction(_ action: MessageItemAction) {
        guard let buttonLink = action.url else {
            Sendsay.logger.log(.error, message: "AppInbox action \"\(action.title ?? "<nil>")\" contains invalid browser link \(action.url ?? "<nil>")")
            return
        }
    ...
    func openDeeplinkAction(_ action: MessageItemAction) {
        guard let buttonLink = action.url else {
            Sendsay.logger.log(.error, message: "AppInbox action \"\(action.title ?? "<nil>")\" contains invalid universal link \(action.url ?? "<nil>")")
            return
        }
...
private extension AppInboxDetailViewController {
    func convertToDarkIfNeeded() {
        guard Sendsay.shared.isDarkMode else { return }
        if #available(iOS 13.0, *) {
    ...
    func setupLayout() {
        ...
        messageImage
            ...
            .padding(.top, constant: Sendsay.shared.configuration?.appInboxDetailImageInset ?? 0)
