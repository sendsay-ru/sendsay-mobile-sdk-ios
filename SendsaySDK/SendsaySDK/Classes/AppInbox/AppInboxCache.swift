...
final class AppInboxCache: AppInboxCacheType {
    static let appInboxFolder = "sendsaysdk_app_inbox"
        ...
        if let directory = getCacheDirectoryURL(),
           let data = try? Data(contentsOf: directory.appendingPathComponent(AppInboxCache.appInboxFileName)) {
            do {
                ...
                Sendsay.logger.log(.error, message: "Error getting stored AppInbox messages \(error.localizedDescription)")
                resultData = AppInboxData()
            }
        } else {
            Sendsay.logger.log(.error, message: "Error getting stored AppInbox messages: Unable to create directory")
            resultData = AppInboxData()
        }
        return resultData
    }
    ...
    private func storeData() {
        guard
            ...
            let directory = getCacheDirectoryURL() else {
                Sendsay.logger.log(.error, message: "Unable to serialize AppInbox data.")
                return
        }
        do {
            ...
            Sendsay.logger.log(.error, message: "Saving AppInbox to file failed: \(error.localizedDescription)")
        }
    }
    ...
    func hasImageData(at imageUrl: String) -> Bool {
        guard let directory = getCacheDirectoryURL() else {
            Sendsay.logger.log(.warning, message: "Unable to get AppInbox image cache directory")
            return false
        }
        let fileUrl = directory.appendingPathComponent(FileUtils.getFileName(fileUrl: imageUrl))
        let exists = fileManager.fileExists(atPath: fileUrl.path)
        if !exists {
            Sendsay.logger.log(.verbose, message: "AppInbox image \(imageUrl) not found in cache.")
        }
        return exists
    }
