...
private extension AppInboxListViewController {
    ...
    func setupElements() {
        ...
        title = NSLocalizedString("sendsay.inbox.title", value: "Inbox", comment: "")
        ...
        statusEmptyTitle.text = NSLocalizedString("sendsay.inbox.emptyTitle", value: "Empty Inbox", comment: "")
        statusEmptyMessage.font = .systemFont(ofSize: 14)
        statusEmptyMessage.numberOfLines = 0
        statusEmptyMessage.text = NSLocalizedString("sendsay.inbox.emptyMessage", value: "You have no messages yet.", comment: "")

        statusErrorTitle.font = .boldSystemFont(ofSize: 20)
        statusErrorTitle.numberOfLines = 0
        statusErrorTitle.text = NSLocalizedString("sendsay.inbox.errorTitle", value: "Something went wrong :(", comment: "")
        statusErrorMessage.font = .systemFont(ofSize: 14)
        statusErrorMessage.numberOfLines = 0
        statusErrorMessage.text = NSLocalizedString("sendsay.inbox.errorMessage", value: "We could not retrieve your messages.", comment: "")
    }
    ...
    func loadMessages() {
        showLoading()
        Sendsay.shared.fetchAppInbox { [weak self] result in
    ...
    private func onMessageItemClicked(_ message: MessageItem, _ index: Int) {
        let detailView = Sendsay.shared.getAppInboxDetailViewController(message.id)
        Sendsay.shared.trackAppInboxOpened(message: message)
...
extension AppInboxListViewController: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return Sendsay.shared.appInboxProvider.getAppInboxListTableViewCell(cell)
    }
}
...
extension AppInboxListViewController: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        Sendsay.shared.markAppInboxAsRead(message) { [weak self] marked in
            guard let self = self else { return }
