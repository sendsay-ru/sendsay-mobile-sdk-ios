...
final class AppInboxManager: AppInboxManagerType {
    ...
    func onEventOccurred(of type: EventType, for event: [DataType]) {
        ...
        if type == .identifyCustomer {
            Sendsay.logger.log(.verbose, message: "CustomerIDs are updated, clearing AppInbox messages")
            clear()
    ...
    func fetchAppInbox(customerIds: [String: String]?, completion: @escaping (Result<[MessageItem]>) -> Void) {
        DispatchQueue.global(qos: .background).async { [weak self] in
            guard let self = self else {
                Sendsay.logger.log(.error, message: "Fetching AppInbox stops due to obsolete thread")
                completion(.failure(SendsayError.stoppedProcess))
                return
            }
            self.isFetching = true
            let customerIds = customerIds ?? self.trackingManager.customerIds
            guard !IntegrationManager.shared.isStopped else {
                Sendsay.logger.log(.error, message: "AppInbox fetch failed, SDK is stopping")
                completion(.failure(SendsayError.stoppedProcess))
                return
            }
            self.repository.fetchAppInbox(
                ...
                case .success(let response):
                    guard self.savedCustomerIds.last == nil || Sendsay.shared.trackingManager?.customerIds == self.savedCustomerIds.last else {
                        self.clear()
                    ...
                    self.savedCustomerIds.removeAll()
                    Sendsay.logger.log(.verbose, message: "AppInbox loaded successfully")
                ...
                case .failure(let error):
                    Sendsay.logger.log(.error, message: "AppInbox loading failed. \(error.localizedDescription)")
                    print(error)
    ...
    func fetchAppInboxItem(_ messageId: String, completion: @escaping (Result<MessageItem>) -> Void) {
        ...
        DispatchQueue.global(qos: .background).async { [weak self] in
            guard let self = self else {
                Sendsay.logger.log(.error, message: "Fetch AppInbox item stops due to obsolete thread")
                completion(.failure(SendsayError.stoppedProcess))
                return
            }
            ...
            self.fetchAppInbox { _ in
                ...
                Sendsay.logger.log(.warning, message: "AppInbox message \(messageId) not found")
                DispatchQueue.main.async {
    ...
    func markMessageAsRead(_ message: MessageItem, _ customerIdsCheck: TypeBlock<Bool>? = nil, _ completition: ((Bool) -> Void)?) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "AppInbox message \(message.id) not read, SDK is stopping")
            return
        }
        DispatchQueue.global(qos: .background).async { [weak self] in
            guard let self = self else {
                Sendsay.logger.log(.error, message: "MarkAsRead AppInbox stops due to obsolete thread")
                DispatchQueue.main.async {
                    completition?(false)
                }
                return
            ...
            guard !message.customerIds.isEmpty, let syncToken = message.syncToken else {
                Sendsay.logger.log(.error, message: "Unable to mark message \(message.id) as read, try to fetch AppInbox")
                DispatchQueue.main.async {
                    completition?(false)
                }
                return
