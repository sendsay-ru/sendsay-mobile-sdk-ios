...
open class DefaultAppInboxProvider: AppInboxProvider {
    ...
    private lazy var appInboxButton: UIButton = {
        ...
        if let imageData = Data.init(base64Encoded: APPINBOX_BUTTON_ICON_DATA, options: .init(rawValue: 0)),
           let trayDown = UIImage(data: imageData, scale: 3) {
            button.setImage(trayDown.withTintColor(.colorPrimaryDark), for: .normal)
        }
        button.setTitle(NSLocalizedString(
            "sendsay.inbox.button",
            value: "Inbox",
            comment: ""
        ), for: .normal)
        button.setTitleColor(.colorPrimaryDark, for: .normal)
        button.addTarget(Sendsay.shared, action: #selector(Sendsay.shared.openAppInboxList), for: .touchUpInside)
        return button
    }()
    ...
    open func getAppInboxDetailViewController(_ messageId: String) -> UIViewController {
        ...
        Sendsay.shared.fetchAppInboxItem(messageId) { result in
            switch result {
