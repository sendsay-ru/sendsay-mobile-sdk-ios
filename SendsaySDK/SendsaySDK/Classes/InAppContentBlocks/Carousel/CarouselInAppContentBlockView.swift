...
open class CarouselInAppContentBlockView: UIView {
    ...
    open func reload(isTriggered: Bool = false) {
        ...
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "In-app reload failed: SDK is stopping")
            return
        }
    ...
    private func listenToState() {
        $state
            ...
            .sink { [weak self] state in
                guard let self else { return }
                guard !IntegrationManager.shared.isStopped else {
                    Sendsay.logger.log(.error, message: "In-app content blocks fetch failed: SDK is stopping")
                    stopTimer()
                    return
                }
                switch state {
...
extension CarouselInAppContentBlockView: UICollectionViewDelegateFlowLayout {
    public func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if let index = messages.firstIndex(where: { $0.message?.id == messageResponse.id }) {
            ...
            Sendsay.logger.log(.error, message: "Error while calling onMessageShown callback, index of message not found")
        }
