...
extension InAppContentBlocksManager: InAppContentBlocksManagerType, WKNavigationDelegate {    
    func hasHtmlImages(html: String) -> Bool {
        ...
        if !isAnyCorrectImage {
            Sendsay.logger.log(.warning, message: "No correct images inside \(html)")
        }
    ...
    func webView(
        ...
        let webAction: WebActionManager = .init { _ in
            ...
            Sendsay.logger.log(.error, message: errorMessage)
    ...
    private func invokeActionInternally(_ action: InAppContentBlockAction) {
        ...
        default:
            Sendsay.logger.log(.warning, message: "No AppInbox action for type \(action.type)")
        }
    }

    func openBrowserAction(_ action: InAppContentBlockAction) {
        guard let buttonLink = action.url else {
            Sendsay.logger.log(.error, message: "AppInbox action \"\(action.name ?? "<nil>")\" contains invalid browser link \(action.url ?? "<nil>")")
            return
        }
    ...
    func openDeeplinkAction(_ action: InAppContentBlockAction) {
        guard let buttonLink = action.url else {
            Sendsay.logger.log(.error, message: "AppInbox action \"\(action.name ?? "<nil>")\" contains invalid universal link \(action.url ?? "<nil>")")
            return
        }
    ...
    func prefetchPlaceholdersWithIds(ids: [String]) {
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks prefetch starts.")
        guard let customerIds = try? DatabaseManager().currentCustomer.ids, !ids.isEmpty else {
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks prefetch starts failed due to customer ids or ids are empty")
            return
        }
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks prefetch ids \(ids)")
        provider.loadPersonalizedInAppContentBlocks(
            ...
            ensureBackground {
                ...
                Sendsay.logger.log(.verbose, message: "In-app Content Blocks downloaded prefetched messages \(prefetchedMessagesDescriptions)")
    ...
    func getFilteredMessage(message: InAppContentBlockResponse) -> Bool {        
        ...
        case .oncePerVisit:
            ...
            if !shouldDisplay {
                Sendsay.logger.log(.verbose, message: "In-app Content Blocks '\(message.name)' already displayed.")
            }
            return shouldDisplay
        case .onlyOnce:
            let shouldDisplay = displayState.displayed ?? Date(timeIntervalSince1970: 0) < sessionStart
            if !shouldDisplay {
                Sendsay.logger.log(.verbose, message: "In-app Content Blocks '\(message.name)' already displayed this session.")
            }
            return shouldDisplay
        case .untilVisitorInteracts:
            let shouldDisplay = displayState.interacted == nil
            Sendsay.logger.log(.verbose, message: "shouldDisplay \(shouldDisplay) for id \(message.id)")
            if !shouldDisplay {
                Sendsay.logger.log(.verbose, message: "In-app Content Blocks '\(message.name)' already interacted with.")
            }
            return shouldDisplay
        case .always:
            return true
        case .none:
            Sendsay.logger.log(.warning, message: "Unknown inAppContentBlocks message frequency.")
            return true
        }
    }
    ...
    private func markAsActive(message: InAppContentBlockResponse, indexPath: IndexPath, placeholderId: String) {
        ...
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks markAsActive indexPath: \(indexPath), placeholderId: \(placeholderId).")
        _usedInAppContentBlocks.changeValue(with: { $0[placeholderId] = blocksToReturn })
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks updated \(usedInAppContentBlocks.mapValues { $0.map { $0.describeDetailed() } })")
    }
    ...
    private func markAsInactive(indexPath: IndexPath, placeholderId: String) {
        ...
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks markAsInactive indexPath: \(indexPath), placeholderId: \(placeholderId).")
        _usedInAppContentBlocks.changeValue(with: { $0[placeholderId] = blocksToReturn })
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks updated \(usedInAppContentBlocks.mapValues { $0.map { $0.describeDetailed() } })")
    }
    ...
    func prepareInAppContentBlockView(placeholderId: String, indexPath: IndexPath) -> UIView {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.verbose, message: "In-app content blocks fetch failed: SDK is stopping")
            return .init()
        }
        ...
        guard messagesNeedToRefresh.isEmpty && expiredMessages.isEmpty else {
            Sendsay.logger.log(.verbose, message: "Loading content for In-app Content Block with placeholder: \(placeholderId) and indxPath \(indexPath)")
        ...
        guard let message = filterPersonalizedMessages(input: messagesToFilter) else {
            Sendsay.logger.log(.verbose, message: "No more In-app Content Block messages for indexPath  \(indexPath)")
            markAsInactive(indexPath: indexPath, placeholderId: placeholderId)
            return returnEmptyView(tag: Int.random(in: 0..<99999999))
        }
        Sendsay.logger.log(.verbose, message: "Filtered In-app Content Block \(message.describe())")
        ...
        if let html = message.content?.html, !html.isEmpty {
            ...
            if inAppContentBlockMessages[indexOfPlaceholder].normalizedResult == nil {
                ...
                Sendsay.logger.log(.verbose, message: "In-app Content Block prepareInAppContentBlockView normalizeConf \(normalizeConf)")
    ...
    func prepareInAppContentBlocksStaticView(
        ...
        guard var message = filterPersonalizedMessages(input: placehodlersToUse.filter { $0.personalizedMessage?.status == .ok }) else {
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks prepareInAppContentBlocksStaticView message not found.")
            return .init(html: "", tag: 0, message: nil)
        }
        Sendsay.logger.log(
            .verbose,
        ...
        let tag = createUniqueTag(placeholder: message)
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks prepareInAppContentBlocksStaticView tag \(tag).")
...
private extension InAppContentBlocksManager {
    func loadPersonalizedInAppContentBlocks(for placeholderId: String, tags: Set<Int>, skipLoad: Bool = false, completion: EmptyBlock?) {
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks loadPersonalizedInAppContentBlocks starts")
        guard !placeholderId.isEmpty, let ids = try? DatabaseManager().currentCustomer.ids else {
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks loadPersonalizedInAppContentBlocks failed placeholderId.isEmpty: \(placeholderId.isEmpty) and ids: \(String(describing: try? DatabaseManager().currentCustomer.ids))")
            return
        }
    ...
    internal func applyDateFilter(message: InAppContentBlockResponse) -> Bool {
        ...
        if let start = message.dateFilter.fromDate, start > Date() {
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks '\(message.name)' outside of date range.")
            return false
        }
    ...
    func loadContent(indexPath: IndexPath, placeholder: String, expired: [InAppContentBlockResponse]) {
        guard let ids = try? DatabaseManager().currentCustomer.ids else {
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks loadContent - customer ids not found")
            return
        }
        if !isLoadUpdating {
            isLoadUpdating = true
            ...
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks placeholdersNeedToGetContent count \(placeholdersNeedToGetContent.count)")
            Sendsay.logger.log(
                .verbose,
                message: "In-app Content Blocks placeholdersNeedToGetContent \(placeholdersNeedToGetContent.map { $0.describe() })"
            )
            Sendsay.logger.log(.verbose, message:
                """
    ...
    func calculateStaticData(height: CalculatorData, newValue: UsedInAppContentBlocks, placeholder: InAppContentBlockResponse) {
        ...
        if savedInAppContentBlocksToDeactived.isEmpty {
            ...
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks usedInAppContentBlocks \(usedInAppContentBlocks.mapValues { $0.map { $0.describeDetailed() } })")
...
extension InAppContentBlocksManager {
    private func continueWithStaticQueue() {
        ...
        if !staticQueue.isEmpty {
            ...
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks continueWithStaticQueue \(go)")
    ...
    private func continueWithCarouselQueue(dataCompletion: TypeBlock<[StaticReturnData]>?) {
        ...
        if !carouselQueue.isEmpty {
            ...
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks carousel queue \(go)")
    ...
    func loadMessagesForCarousel(placeholder: String, completion: EmptyBlock?) {
        guard !placeholder.isEmpty, let ids = try? DatabaseManager().currentCustomer.ids else {
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks Carousel cant refresh placeholderId: \(placeholder), ids: \(String(describing: try? DatabaseManager().currentCustomer.ids))")
            return
        }
        ...
        provider.loadPersonalizedInAppContentBlocks(
            ...
            ensureBackground {
                ...
                Sendsay.logger.log(.verbose, message: "In-app Content Blocks refreshStaticViewContent data: \(refreshStaticViewContentDescriptions)")
                let personalizedWithPayload: [PersonalizedInAppContentBlockResponse] = data.data?.data.compactMap { response in
                    var newInAppContentBlocks = response
                    let normalizeConf = HtmlNormalizerConfig(
                        makeResourcesOffline: true,
    ...
    func refreshCarouselData(placeholder: String, dataCompletion: TypeBlock<[StaticReturnData]>?) {
        Sendsay.logger.log(.verbose, message: "In-app Content Blocks refreshStaticViewContent")
    ...
    func refreshStaticViewContent(staticQueueData: StaticQueueData) {
        ...
        if !isStaticUpdating {
            ...
            guard !staticQueueData.placeholderId.isEmpty, let ids = try? DatabaseManager().currentCustomer.ids else {
                ...
                Sendsay.logger.log(.verbose, message: "In-app Content Blocks cant refresh static content staticQueueData.placeholderId: \(staticQueueData.placeholderId), ids: \(String(describing: try? DatabaseManager().currentCustomer.ids))")
                isStaticUpdating = false
                return
            }
...
private extension InAppContentBlocksManager {
    func continueWithQueue() {
        ...
        if !queue.isEmpty {
            ...
            Sendsay.logger.log(.verbose, message: "In-app Content Blocks continueWithQueue data: \(go.describeDetailed())")
        ...
        if !isUpdating {
            ...
            loadPersonalizedInAppContentBlocks(for: savedNewValue.messageId, tags: [savedNewValue.tag], skipLoad: true) { [weak self] in
                ...
                self.calculator.heightUpdate = { height in
                    ...
                    Sendsay.logger.log(.verbose, message: "In-app Content Blocks loadContentForPlacehoder calculator data \(height)")
                    // Update display status
                    ...
                    Sendsay.logger.log(.verbose, message: "In-app Content Blocks loadContentForPlacehoder count \(self.inAppContentBlockMessages.count)")
                    Sendsay.logger.log(.verbose, message: "In-app Content Blocks loadContentForPlacehoder \(self.inAppContentBlockMessages.map { $0.describe() })")
                    Sendsay.logger.log(.verbose, message:
                        """
...
extension InAppContentBlocksManager {
    ...
    func updateInteractedState(for messageId: String) {
        Sendsay.shared.inAppContentBlockStatusStore.didInteract(with: messageId, at: Date())
    }
    ...
    func updateDisplayedState(for messageId: String) {
        Sendsay.shared.inAppContentBlockStatusStore.didDisplay(of: messageId, at: Date())
    }

    func getDisplayState(of messageId: String) -> InAppContentBlocksDisplayStatus {
        Sendsay.shared.inAppContentBlockStatusStore.status(for: messageId)
    }
}
