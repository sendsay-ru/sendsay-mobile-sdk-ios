...
public final class StaticInAppContentBlockView: UIView, WKNavigationDelegate {
    ...
    private func getContent(force: Bool = false) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.verbose, message: "In-app content blocks UI is unavailable, SDK is stopping")
            return
        }
    ...
    public func invokeActionClick(actionUrl: String) {
        Sendsay.logger.log(.verbose, message: "InAppCB: Manual action \(actionUrl) invoked on placeholder \(placeholder)")
        _ = handleUrlClick(actionUrl.cleanedURL())
    }

    private func handleUrlClick(_ actionUrl: URL?) -> Bool {
        guard let actionUrl else {
            Sendsay.logger.log(.warning, message: "InAppCB: Unknown action URL: \(String(describing: actionUrl))")
            return false
        }
        ...
        guard let message = assignedMessage else {
            Sendsay.logger.log(.error, message: "InAppCB: Placeholder \(placeholder) has invalid state - action or message is invalid")
        ...
        let webAction: WebActionManager = .init { _ in
            ...
            Sendsay.logger.log(.error, message: "WebActionManager error \(error.localizedDescription)")
        }
        ...
        if handled {
            Sendsay.logger.log(.verbose, message: "[HTML] Action \(actionUrl.absoluteString) has been handled")
        } else {
            Sendsay.logger.log(.verbose, message: "[HTML] Action \(actionUrl.absoluteString) has not been handled, continue")
        }
        return handled
    }
