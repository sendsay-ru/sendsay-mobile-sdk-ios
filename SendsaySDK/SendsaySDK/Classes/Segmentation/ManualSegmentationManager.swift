...
public final class ManualSegmentationManager: ManualSegmentationManagerType {
        ...
        dataProvider.getSegmentations(
            ...
            guard let self, let data = response.data else {
                if let error = response.error {
                    Sendsay.logger.log(.error, message: "Segments: Fetch of segments failed: \(error)")
                }
    ...
    public func getSegments(
        ...
        guard !isRequestRunning else {
            Sendsay.logger.log(.verbose, message: "Segments: Manual fetch is already in progress, waiting for result")
            return
        }
        guard let customerIds = Sendsay.shared.trackingManager?.customerIds, let cookie = customerIds["cookie"] else {
            Sendsay.logger.log(.verbose, message: "Segments: Customer in not identified")
            result([])
            return
        }
        invokeFetch(
    ...
    private func invokeFetch(
        ...
        guard areCustomerIdsEqual(customerIds: customerIds) else {
            invokeFetch(
                customerIds: Sendsay.shared.trackingManager?.customerIds ?? [:],
                category: category,
                result: result,
                cookie: cookie
            )
    ...
    public func anonymize() {
        ...
        Sendsay.logger.log(.verbose, message: "Segments: Segments change check has anonymized")
    }
}
