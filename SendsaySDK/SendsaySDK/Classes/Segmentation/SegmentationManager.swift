...
public final class SegmentationManager: SegmentationManagerType {
    ...
    private var areCustomerIdsEqual: Bool {
        guard let currentCustomerIds = Sendsay.shared.trackingManager?.customerIds else { return false }
        let areSame = currentCustomerIds.compareWith(other: savedCustomerIds)
        if !areSame {
            Sendsay.logger.log(.verbose, message: "Segments: Check process was canceled because customer has changed")
        }
        return areSame
    }
    private var externalIds: [String: String] {
        guard var ids = Sendsay.shared.trackingManager?.customerIds else { return [:] }
        ids["cookie"] = nil
        return ids
    }
...
extension SegmentationManager {
    ...
    public func anonymize() {
        ...
        storedSegmentations = nil
        debouncer.stop()
        Sendsay.logger.log(.verbose, message: "Segments: Segments change check has been cancelled meanwhile")
    }
    ...
    public func processTriggeredBy(type: SegmentTriggerType) {
        ...
        case .`init`:
            ...
            if callbacks.isEmpty {
                Sendsay.logger.log(.verbose, message: "Segments: Skipping initial segments update process for no callback")
            }
            processFetch()
        case let .register(data):
            ...
            if Sendsay.shared.isConfigured {
                processFetch()
            }
        case .identify:
            processFetch()
    ...
    private func processFetch() {
        guard !callbacks.isEmpty else {
            Sendsay.logger.log(.verbose, message: "Segments: Skipping segments update process after tracked event due to no callback registered")
            return
        }
        segmentChangeCheck()
    }
    ...
    private func segmentChangeCheck() {
        savedCustomerIds = Sendsay.shared.trackingManager?.customerIds ?? [:]
        debouncer.debounce { [weak self] in
            guard let self else { return }
            let currentCusomterIds = Sendsay.shared.trackingManager?.customerIds ?? [:]
            let currentNewbies = self.newbieCallbacks
            if currentNewbies.isEmpty {
                Sendsay.logger.log(.verbose, message: "Segments: Skipping initial segments update process as is not required")
            }
    ...
    private func getSegments(customerIds: [String: String], newbies: [SegmentCallbackData]) {
        guard !IntegrationManager.shared.isStopped else {
            Sendsay.logger.log(.error, message: "Segments fetch failed, SDK is stopping")
            return
        }
        ...
        dataProvider.getSegmentations(data: SegmentDataDTO.self, cookie: cookie) { [weak self] response in
            ...
            guard let self,
                ...
                if let error = response.error {
                    Sendsay.logger.log(.verbose, message: "Segments: Fetch of segments failed: \(error)")
                } else {
                    Sendsay.logger.log(.verbose, message: "Segments: New data are ignored because were loaded for different customer")
                }
                return
            }
            ...
            guard !union.isEmpty else {
                Sendsay.logger.log(.verbose, message: "Segments: Empty data from server and store.")
            ...
            union.forEach { category in
                guard !self.callbacks.isEmpty else {
                    Sendsay.logger.log(.verbose, message: "Segments: Skipping segments reload process for no callback")
                    return
                }
    ...
    private func linkIds(completion: @escaping EmptyBlock) {
        guard let cookie = Sendsay.shared.customerCookie, let ids = Sendsay.shared.trackingManager?.customerIds else { return }
        dataProvider.linkIds(data: EmptyDTO.self, cookie: cookie, externalIds: externalIds) { response in
            if response.error != nil {
                Sendsay.logger.log(.warning, message: "Segments: Customer IDs \(ids) merge failed, unable to fetch segments due to \(String(describing: response.error?.localizedDescription))")
            }
            completion()
        }
    }
