...
@objc(IsSendsaySDKTest)
protocol IsSendsaySDKTest {
}

final class MockSendsayImplementation: SendsayInternal {
    ...
    // override Sendsay sharedInitializer to add mock database manager insead of coreData
    public override func sharedInitializer(configuration: Configuration) {
        Sendsay.logger.log(.verbose, message: "Configuring MockSendsay with provided configuration:\n\(configuration)")
        ...
        do {
            ...
            Sendsay.logger.log(.error, message: """
                Error while creating a database, MockSendsay cannot be configured.\n\(error.localizedDescription)
                """)
        }
    }
