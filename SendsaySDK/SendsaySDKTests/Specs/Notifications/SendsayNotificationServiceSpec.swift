//
//  SendsayNotificationServiceSpec.swift
//  SendsaySDKTests
//
...
final class SendsayNotificationServiceSpec: QuickSpec {
    ...
    private func getRecordedNotifications() -> [Data]? {
        ...
        if let delivered = userDefaults.array(forKey: SendsaySDK.Constants.General.deliveredPushUserDefaultsKey)
           as? [Data] {
            return delivered
        }
        return nil
    ...
    private func getRecordedNotificationEvents() -> [Data]? {
        let userDefaults = UserDefaults(suiteName: "mock-app-group")!
        if let delivered = userDefaults.array(forKey: SendsaySDK.Constants.General.deliveredPushEventUserDefaultsKey)
           as? [Data] {
            return delivered
        }
        return nil
        ...
        describe("processing") {
            ...
            it("should not save notification events for later when tracking failed by network") {
                try! Configuration(
                    projectToken: "mock-project-token",
                    projectMapping: nil,
                    authorization: SendsaySDK.Authorization.token("mock-token"),
                    baseUrl: nil,
                    appGroup: "mock-app-group",
                    defaultProperties: nil
                ).saveToUserDefaults()
                guard let userDefaults = UserDefaults(suiteName: "mock-app-group"),
                    let data = try? JSONEncoder().encode(["uuid": SendsaySDK.JSONValue.string("mock-uuid")]) else {
                    return
                }
