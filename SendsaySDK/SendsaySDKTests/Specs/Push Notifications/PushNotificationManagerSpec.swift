...
@testable import SendsaySDKNotifications
...
final class PushNotificationManagerSpec: QuickSpec {
    ...
    private class VerifingPushNotificationManagerDelegate: PushNotificationManagerDelegate {
        private let action: SendsaySDK.SendsayNotificationActionType
        private let value: String?
        private let extraData: [AnyHashable: Any]?
        init(
            action: SendsaySDK.SendsayNotificationActionType,
            value: String?,
        ...
        func pushNotificationOpened(
            with action: SendsaySDK.SendsayNotificationActionType,
            value: String?,
            extraData: [AnyHashable: Any]?
        ) {
    ...
    override func spec() {
        ...
        func createPushManager(
            ...
            currentToken: String?,
            tokenTrackFrequency: SendsaySDK.TokenTrackFrequency,
        ...
        describe("tracking stored delivered push notifications") {
            func getFirstStoredNotification() -> SendsaySDK.NotificationData? {
                let userDefaults = UserDefaults(suiteName: "mock-app-group")!
                let data = userDefaults.array(
                    forKey: SendsaySDK.Constants.General.deliveredPushUserDefaultsKey
                ) as? [Data]
            ...
            func getFirstStoredNotificationEvent() -> EventTrackingObject? {
                let userDefaults = UserDefaults(suiteName: "mock-app-group")!
                let data = userDefaults.array(
                    forKey: SendsaySDK.Constants.General.deliveredPushEventUserDefaultsKey
                ) as? [Data]
                guard let delivered = data, !delivered.isEmpty else {
                    return nil
                }
            ...
            testCases.forEach { testCase in
                it("should track notification with \(testCase.name)") {
                    let service = SendsayNotificationService(appGroup: "mock-app-group")
            ...
            let testCases: [NotificationOpenedTestCase] = [
                ...
                NotificationOpenedTestCase(
                    ...
                    delegate: VerifingPushNotificationManagerDelegate(
                        action: SendsaySDK.SendsayNotificationActionType.openApp,
                        value: nil,
                        extraData: nil
                    ),
                    expectedTrackedEvent: MockTrackingManager.TrackedEvent(
                ...
                NotificationOpenedTestCase(
                    ...
                    actionIdentifier: "SENDSAY_ACTION_APP_2",
                    delegate: VerifingPushNotificationManagerDelegate(
                        action: SendsaySDK.SendsayNotificationActionType.browser,
                    ...
                    expectedTrackedEvent: MockTrackingManager.TrackedEvent(
                        ...
                        data: [.properties([
                            ...
                            "utm_source": JSONValue.string("sendsay"),
                            "utm_campaign": JSONValue.string("Testing mobile push"),
                            "utm_medium": JSONValue.string("mobile_push_notification")
                        ] as [String: SendsaySDK.JSONValue]), // without swift fails with typechecking took too long
                        .timestamp(PushNotificationsTestData.timestamp),
                        .eventType("campaign")]
                    ),
        ...
        describe("saving opened push for later") {
            ...
            it("should save opened push for later") {
                ...
                let optionalDataArray = UserDefaults(suiteName: SendsaySDK.Constants.General.userDefaultsSuite)?
                    .array(forKey: SendsaySDK.Constants.General.openedPushUserDefaultsKey) as? [Data]
                ...
                UserDefaults(suiteName: SendsaySDK.Constants.General.userDefaultsSuite)?
                    .removeObject(forKey: SendsaySDK.Constants.General.openedPushUserDefaultsKey)
            }
            ...
            it("should process previously saved push notifications") {
                ...
                expect(
                    UserDefaults(suiteName: SendsaySDK.Constants.General.userDefaultsSuite)?
                        .array(forKey: SendsaySDK.Constants.General.openedPushUserDefaultsKey)
                ).to(beNil())
                expect(trackingManager.trackedEvents.count).to(equal(2))
                expect(trackingManager.trackedEvents[0].type).to(equal(SendsaySDK.EventType.pushOpened))
                expect(trackingManager.trackedEvents[1].type).to(equal(SendsaySDK.EventType.pushDelivered))
            }
        }
