            ...
            it("should return true for sdk related exception") {
                expect(
                    TelemetryUtility.isSDKRelated(
                        stackTrace: ["something", "something", "something Sendsay something", "something"]
                    )
                ).to(beTrue())
                expect(
                    TelemetryUtility.isSDKRelated(
                        stackTrace: ["something", "something", "xxxsendsayxxx", "something"]
                    )
                ).to(beTrue())
            }
            it("should return true for sdk related exception") {
            ...
            it("should format non-default configuration") {
                let configuration = try! Configuration(
                    ...
                    projectMapping: [EventType.banner: [
                        SendsayProject(projectToken: "other-mock-project-token", authorization: .none)
                    ]],
