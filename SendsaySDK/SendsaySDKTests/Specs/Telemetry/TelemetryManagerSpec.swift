    ...
    override func spec() {
        ...
        it("should report sdk/swift error") {
            ...
            expect(self.upload.uploadedCrashLogs[0].errorData.message)
                .to(equal("SendsaySDK.DatabaseManagerError:5 Object does not exist."))
        ...
        it("should report event") {
            ...
            expect(self.upload.uploadedEvents[0].properties["sdkVersion"]).to(equal(Sendsay.version))
            expect(self.upload.uploadedEvents[0].properties["appNameVersionSdkVersion"])
                .to(equal("com.apple.dt.xctest.tool - \(appVersion) - SDK \(Sendsay.version)"))
