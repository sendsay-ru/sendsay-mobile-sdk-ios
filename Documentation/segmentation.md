...
The SDK provides an API to get segmentation data directly. Invoke the `Sendsay.shared.getSegments` method, passing a `category` value as argument:

```swift
Sendsay.shared.getSegments(category: .content()) { segments in
    print(segments)
}
```
...
```swift
    Sendsay.shared.getSegments(force: true, category: .discovery()) { data in
        print(data)
    }
```
...
> 👍
>
> The `getSegments` method loads segmentation data for the requested `category` and the current customer as identified by `Sendsay.shared.identifyCustomer`. Please bear in mind that the callback is invoked in a background thread.
...
3. When `Sendsay.shared.identifyCustomer` is called with a [hard ID](https://documentation.bloomreach.com/engagement/docs/customer-identification#section-hard-id).
...
> ❗️
>
> It is required to [set](https://documentation.bloomreach.com/engagement/docs/ios-sdk-data-flushing#flushing-modes) `Sendsay.shared.flushingMode` to `IMMEDIATE` to get accurate results. The process of segment calculation needs all tracked events to be uploaded to server to calculate results effectively.
...
>
> The SDK logs a lot of useful information related to segmentation data updates on the `VERBOSE` level. You can set the logger level using `Sendsay.logger.logLevel` before initializing the SDK.
...
If you are not receiving segmentation data while registering a customer, please check your usage of `SENDSAYSendsay.shared.identifyCustomer` or `Sendsay.shared.anonymize`. You may face these logs:

- ```
  Segments: Segments change check has been cancelled meanwhile
  ```
  The segmentation data update process started but was subsequently canceled by an invocation of `Sendsay.shared.anonymize`. If this is unwanted behavior, check your `Sendsay.shared.anonymize` usage.
- ```
  Segments: Check process was canceled because customer has changed
  ```
  The segmentation data update process started for the current customer but the customer ID was subsequently changed by an invocation of `Sendsay.shared.identifyCustomer` for another customer. If this is unwanted behaviour, check your `Sendsay.shared.identifyCustomer` usage.
- ```
  Segments: Customer IDs <customer_ids> merge failed, unable to fetch segments
  ```
  The segmentation data update process requires to link IDs but that part of the process failed. Please refer to the error log messages and check your `Sendsay.identifyCustomer` usage. This  should not happen, please discuss this with the Bloomreach support team.
- ```
  Segments: New data are ignored because were loaded for different customer
  ```
  The segmentation data update process detects that data has been fetched for a previous customer. This should not lead to any problem as there is another fetch process registered for the new customer, but you may face a short delay for new data retrieval. If you see this log often, check your `Sendsay.shared.identifyCustomer` usage.
- ```
