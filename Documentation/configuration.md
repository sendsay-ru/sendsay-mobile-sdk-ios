...
* `manualSessionAutoClose`
  * Determines whether the SDK automatically tracks `session_end` for sessions that remain open when `Sendsay.shared.trackSessionStart()` is called multiple times in manual session tracking mode.
  * Default value: `true`

## Configure the SDK

### Configure the SDK programmatically
Configuration is split into several objects that are passed into the `Sendsay.shared.configure()` function.
``` swift
func configure(
...
* `pushNotificationTracking` **(required)**
  ...
  * Setting `delegate` has the same effect as setting `Sendsay.shared.pushNotificationsDelegate`.
