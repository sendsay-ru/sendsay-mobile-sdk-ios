---
title: Release notes
excerpt: Sendsay iOS SDK release notes
...
* Bug Fixes
  ...
  * Fixed: Could not find module 'SendsaySDK' for simulator target x86_64
...
#### January 13, 2022
* Features
  * Tracking button URL as link attribute for banner click event [Issue #31](https://github.com/sendsay/sendsay-ios-sdk/issues/31)
* Bug Fixes
  * Fixed: SendsaySDKObjC and SendsaySDKShared frameworks are no longer embedded [Issue #33](https://github.com/sendsay/sendsay-ios-sdk/issues/33)
  ...
  * Fixed: Warnings appearing with XCode12 and above [Issue #35](https://github.com/sendsay/sendsay-ios-sdk/issues/35)
...
* Bug Fixes
  * Fixed: Configuration of base URL works with and without the trailing slash [Issue #24](https://github.com/sendsay/sendsay-ios-sdk/issues/24).
...
* Bug Fixes
  * **BREAKING CHANGE:**
  ...
  `SendsayAppDelegate` now implements this method where it processes the notification and sets notification center delegate. Your `AppDelegate application:didFinishLaunchingWithOptions` now requires a `override` keyword and a **call to super** `super.application(application, didFinishLaunchingWithOptions: launchOptions)`. Calling `Sendsay.shared.pushNotificationsDelegate = self` is no longer required. See [push notifications documentation](./Guide/PUSH_QUICKSTART.md) for more details.
  * Fixed: Events are now sorted based on timestamp before uploading to Sendsay servers.
...
#### September 09, 2020
* Features
  * Default properties that are tracked with every event can be changed at runtime by setting `Sendsay.shared.defaultProperties` property.
...
#### August 07, 2020
* Features
  * Support for new in-app message delay and timeout fields soon to be available in Sendsay web app.
...
* Bug Fixes
  ...
  * **BREAKING CHANGE**: Only strings are now allowed for customer ids. Sendsay would ignore all other types, so this change should not break any functionality, possibly just clean up non-functioning code.
...
#### July 20, 2020
* Features
  * **DEPRECATION NOTICE**: Automatic push notifications are now deprecated. Please check the [Push notification](./PUSH.md) documentation to see new setup instructions. In most cases, just extending `SendsayAppDelegate` should be enough. The reason for this change is to remove method swizzling that causes issues when multiple SDKs that handle push notifications are integrated into one application. This way you're more in control of the push notification flow.
  * Silent push notifications support. You're now able to send background updates to your application and respond to them by implementing `silentPushNotificationReceived` method on `PushNotificationManagerDelegate`. Delivery of silent push notifications is tracked to Sendsay backend.
...
#### June 30, 2020
* Features
  * Internal features required for Sendsay React Native SDK. Developers using Swift/Objective C can ignore this release completely.
...
* Features
  ...
  * Retrieve the cookie of the current customer used for tracking by calling `Sendsay.shared.customerCookie`.
...
* Features
  ...
  * Push notification token is removed from Sendsay servers when user changes push notification permission.
* Bug fixes
...
* Features
  ...
  * The SDK is now able to report the SDK-related crashes to Sendsay. This helps us keep the SDK in a good shape and work on fixes as soon as possible.
* Bug fixes
  ...
  * Removed: Automatic payment tracking was broken and has been removed from the Sendsay iOS SDK. In case you're interested in this functionality let us know.
...
#### November 26, 2019
* Features
  * The SDK can now be fully configured also in the code, not only with the configuration file and thus making it more flexible. This new feature is based on https://github.com/sendsay/sendsay-ios-sdk/issues/10.
* Bug Fixes
  * Fixed: https://github.com/sendsay/sendsay-ios-sdk/issues/8 - A push notification might have opened the application multiple times if there are multiple SDKs integrated in the application. This shouldn't happen anymore.
...
* Features
  ...
  * Push notification events now contain more information about campaigns and the action taken and are consistent with Sendsay Android SDK.
* Bug Fixes
...
* Bug Fixes
  ...
  * Fixed: Event flushing can now only be performed once at a time. Simultaneous calls to `Sendsay.shared.flushData()` will only flush data to Sendsay once. Flush uses semaphore to ensure that only one thread can perform flushing process.

### 2.2.3
* Configuration now has a token update frequency, which specifies how often should the push token be sent to Sendsay (on change, on every start, daily)
...
* Fix issues with notification extension when integrating through CocoaPods
	- **Note:** It is now required to add `pod 'SendsaySDK-Notifications'` to your Podfile instead of the previous `pod 'SendsaySDK/Notifications'`
