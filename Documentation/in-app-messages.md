...
You can override the SDK's default behavior when an in-app message action (click button or close message) is performed by setting `inAppMessagesDelegate` on the `Sendsay` instance.
...
```swift
Sendsay.shared.inAppMessagesDelegate = MyInAppDelegate()
```
...
- The SDK requests in-app messages from the Engagement platform any time one of the following occurs:
  - `Sendsay.identifyCustomer` is called
  - `Sendsay.anonymize` is called
...
- If you create or modify an in-app message in Engagement, typically any changes you made are reflected in the SDK after 30 minutes due to the in-app messages being cached. Call `Sendsay.identifyCustomer` or `Sendsay.anonymize` to trigger reloading so changes are reflected immediately.
...
> ❗️
>
> Invoking `Sendsay.anonymize` triggers fetching in-app messages immediately but `Sendsay.identifyCustomer` needs to be flushed to the backend successfully first. This is because the backend must know the customer so it can assign the in-app messages with matching audience. If you have set `Sendsay.flushMode` to anything other then `FlushMode.IMMEDIATE`, you must call `Sendsay.flushData()` to finalize the `identifyCustomer` process and trigger an in-app messages fetch.
