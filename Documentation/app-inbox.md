...
![App Inbox button](https://raw.githubusercontent.com/sendsay/sendsay-ios-sdk/main/Documentation/images/app-inbox-button.png)

Use the `getAppInboxButton()` method to retrieve the button:

```swift
let button = Sendsay.shared.getAppInboxButton()
```
...
> 📘
>
> See [FetchViewController](https://github.com/sendsay/sendsay-ios-sdk/blob/main/SendsaySDK/Example/Views/Fetching/FetchViewController.swift) in the [example app](https://documentation.bloomreach.com/engagement/docs/ios-sdk-example-app) for a reference implementation.
...
3. Call `Sendsay.shared.trackAppInboxOpened` when the user clicks on a list item and mark the message as read automatically.
...
5. Call `Sendsay.shared.trackAppInboxClick` automatically when the user clicks a button in the message detail view.


![App Inbox messages list view and message detail view](https://raw.githubusercontent.com/sendsay/sendsay-ios-sdk/main/Documentation/images/app-inbox.png)
...
> 📘
>
> Refer to [ExampleAppInboxProvider](https://github.com/sendsay/sendsay-ios-sdk/blob/main/SendsaySDK/Example/Views/ExampleAppInboxProvider.swift) in the [example app](https://documentation.bloomreach.com/engagement/docs/ios-sdk-example-app) for a reference implementation.

#### App Inbox button

The method `getAppInboxButton()` returns a `UIButton` instance.

The default implementation builds a simple button instance with an icon ![Inbox icon](https://raw.githubusercontent.com/sendsay/sendsay-ios-sdk/main/Documentation/images/inbox.png) and the `sendsay.inbox.button` label. The click action for the button opens the App Inbox list view.
...
> ❗️
>
> The methods `Sendsay.shared.trackAppInboxOpened` and `Sendsay.shared.markAppInboxAsRead` are called when the user clicks on an item. Please call these methods in your custom implementation to maintain correct App Inbox behavior.
...
> ❗️
>
> The method `Sendsay.shared.trackAppInboxClick` is called when the user clicks on an action. Please call this method in your custom implementation to maintain correct App Inbox behavior.
...
- `Sendsay.shared.identifyCustomer`
...
```swift
Sendsay.shared.fetchAppInbox { result in
   switch result {
   case .success(let messages):
      if (messages.isEmpty) {
         Sendsay.logger.log(.verbose, message: "App inbox loaded but is empty")
         return
      }
      Sendsay.logger.log(.verbose, message: "App inbox loaded")
   case .failure(let error):
      Sendsay.logger.log(.verbose, message: "App inbox load failed due error \"\(error.localizedDescription)\"")
   }
}
```
...
```swift
Sendsay.shared.fetchAppInboxItem(messageId) { data in
   guard let data = data else {
      Sendsay.logger.log(.error, message: "AppInbox message not found for ID \(messageId)")
      return
   }
   Sendsay.logger.log(.verbose, message: "AppInbox message found and loaded")
}
```
...
```swift
Sendsay.shared.markAppInboxAsRead(messageId) { marked in
   Sendsay.logger.log(.verbose, message: "AppInbox message marked as read: \(marked)")
}
```

> ❗️
>
> Marking a message as read using the `markAppInboxAsRead` method does not trigger a tracking event for opening the message. To track an opened message, you need to call the `Sendsay.shared.trackAppInboxOpened` method). 
...
Use the `Sendsay.shared.trackAppInboxOpened(MessageItem)` method to track opening of App Inbox messages.
...
If you want to ignore tracking consent, use `Sendsay.shared.trackAppInboxOpenedWithoutTrackingConsent` instead. This method will track the event regardless of consent.

#### Track clicked App Inbox message action

Use the `Sendsay.shared.trackAppInboxClick(MessageItemAction, MessageItem)` method to track action invocations in App Inbox messages.
...
If you want to ignore tracking consent, use `Sendsay.shared.trackAppInboxClickWithoutTrackingConsent` instead. This method will track the event regardless of consent.
